...
公开主站使用 Cloudflare Pages 根路径发布，配置与上线契约见
[CLOUDFLARE.md](CLOUDFLARE.md)。GitHub Pages 子路径发布继续保留为回滚通道。
...
- `src/pages/model.astro`：交互预测模型页，展示寿命、有效时间、主观时间、相对时间、未来选择权、合成敏感性分析和模型准入状态；它消费 `life-path-toy-model-audit.json`、L4 evidence packet validator、L4 validation/calibration execution validation 和 NHATS route-classifier synthetic dry-run summary，把 Q3/L3 展示上限、L4 阻塞、0 个真实报告包、0 个人审签名和无个体预测边界显性放到页面。
...
- `src/data/life-path-synthetic-validation-calibration-report.json` / `src/data/life-path-synthetic-validation-calibration-report.md`：由 `npm run export:life-path-validation-calibration-report` 从 toy model、合成敏感性分析和 L4 验证/校准报告契约生成，演练 12 个报告章节和 5 个 L4 槽位如何被填充并继续阻塞；它只证明报告包形状和拒绝规则可运行，不是真实验证、真实校准、L4 准入、公开加权输出或个体预测。
- `src/data/life-path-l4-validation-calibration-report-execution-validation.json`：由 `npm run export:life-path-l4-validation-calibration-report-execution-validation` 从 L4 验证/校准报告执行寄存器生成，验证 12 个报告段落和 5 个 L4WO-05 槽位仍是 pending real report packet、0 个真实报告包、0 个人审签名和 `l4-still-blocked`；它不是模型结果、校准报告、公开加权输出或个体预测。
- `src/data/human-infra-research-standards-source-anchor-validation.json`：由 `npm run export:research-standards-source-anchor-validation` 从外部科研标准锚点注册表生成，确认 TRIPOD+AI、PROBAST+AI、STROBE、RECORD、CONSORT、SPIRIT、CONSORT-AI、SPIRIT-AI、PRISMA、GRADE、RoB 2、ROBINS-I、target trial emulation、STaRT-RWE 和 ISPOR-SMDM 路线已进入机器审计；它只证明标准锚点覆盖和阻塞边界可审计，不证明模型准入、外部验证、个体预测、医学建议或长寿逃逸速度。
- `src/data/human-infra-quantitative-capability-ladder-validation.json`：由 `npm run export:quantitative-capability-ladder-validation` 从 Q0-Q5 定量能力分层生成，确认当前最高能力是 Q3/L3 synthetic life-path model，Q4 聚合校准模型、Q5 个体决策支持、个人死亡日期、医学建议和干预排序仍被阻塞。
- `src/data/human-infra-domain-to-model-bridge-validation.json`：由 `npm run export:domain-to-model-bridge-validation` 从域到模型桥接契约生成，确认 C1-C6 研究域当前只允许作为 Q2/L2 候选变量、候选机制和模型位置进入模型语言；系数、因果效应、校准聚合预测、个体用途、医学建议、干预排序和个人死亡日期输出仍被阻塞。
- `src/data/life-path-l4-synthetic-evidence-packet-dry-run.json` / `src/data/life-path-l4-synthetic-evidence-packet-dry-run.md`：由 `npm run export:life-path-l4-evidence-packet-dry-run` 从 L4 evidence intake、packet review playbook 和合成验证/校准报告 dry-run 生成，演练 24 个 pending slot 的 hash-only draft packet 形状；它只证明证据包 schema 和 abort 边界可运行，不是真实证据包、人审完成、slot closure、L4 准入、公开加权输出或个体预测。
- `src/data/life-path-l4-evidence-packet-validator-validation.json`：由 `npm run export:life-path-l4-evidence-packet-validator-validation` 从 L4 evidence packet validator synthetic cases 生成，验证未来证据包会被判为 `rejected`、`cannot-evaluate` 或 `reviewable-but-still-blocked`；它只证明预检规则可审计，不是真实 evidence packet、人审签名、slot closure、L4 准入、公开加权输出或个体预测。
- `src/data/life-path-nhanes-public-lmf-survey-design-readiness-validation.json`：由 `npm run export:nhanes-survey-design-readiness-validation` 从 NHANES public-use LMF survey-design readiness 契约生成，记录 WTMEC2YR、SDMVPSU、SDMVSTRA 官方字段、gate summary 和 no-weighted-inference 边界；它不证明加权人口推断、design-based interval、校准或个体预测可用。
- `src/data/life-path-nhanes-public-lmf-domain-subpopulation-rule-readiness-validation.json`：由 `npm run export:nhanes-domain-rule-readiness-validation` 从 NHANES public-use LMF domain/subpopulation rule readiness 契约生成，记录 CDC/NCHS domain/subpopulation 机制、禁止 row-drop subgroup filtering、gate summary 和 no-weighted-domain-inference 边界；它不证明 weighted domain inference、design-based interval、校准或个体预测可用。
- `src/data/life-path-nhanes-public-lmf-eligible-base-readiness-validation.json`：由 `npm run export:nhanes-eligible-base-readiness-validation` 从 NHANES public-use LMF positive-weight eligible-base readiness 契约生成，记录 `WTMEC2YR > 0` eligible-base 诊断、5809 名 positive-weight eligible adults、15 个 strata、gate summary 和 no-weighted-domain-inference 边界；它不证明 weighted domain inference、design-based interval、校准或个体预测可用。
- `src/data/life-path-nhanes-public-lmf-weighted-estimator-readiness-validation.json`：由 `npm run export:nhanes-weighted-estimator-readiness-validation` 从 NHANES public-use LMF weighted-estimator readiness 契约生成，记录 R `survey` / `svydesign` 后端选择、design-object / domain indicator 合约、gate summary 和 no-weighted-domain-output 边界；它不证明 R runtime smoke、weighted domain inference、design-based interval、校准或个体预测可用。
- `src/data/life-path-nhanes-public-lmf-r-survey-runtime-smoke-validation.json`：由 `npm run export:nhanes-r-survey-runtime-smoke-validation` 从 NHANES public-use LMF R `survey` runtime smoke readiness 契约生成，记录当前 `Rscript` / `survey` / synthetic `svydesign` smoke 探测状态；它不证明真实 NHANES weighted domain output、design-based interval、校准或个体预测可用。
- `src/data/life-path-nhanes-public-lmf-r-survey-controlled-runtime-smoke-validation.json`：由 `npm run export:nhanes-r-survey-controlled-runtime-smoke-validation` 通过本地 controlled conda runtime 生成，记录 R `4.3.3`、R `survey` 包和 synthetic `svydesign` / domain `subset` smoke 是否可复现；它不进入默认 `audit:life-path-toy`，不处理 NHANES 行级数据，不证明 weighted domain output、design-based interval、校准或个体预测可用。
- `src/data/life-path-nhanes-public-lmf-domain-indicator-diagnostic-validation.json`：由 `npm run export:nhanes-domain-indicator-diagnostic-validation` 从 NHANES public-use LMF domain indicator metadata diagnostic 契约生成，记录 8 个 sex × ageBand 聚合域组合、no-row/no-count/no-weighted-output 边界和继续阻塞的 DOF/披露/加权输出门。
- `src/data/life-path-nhanes-public-lmf-dof-sparse-domain-diagnostic-validation.json`：由 `npm run export:nhanes-dof-sparse-domain-diagnostic-validation` 从 NHANES public-use LMF DOF / sparse-domain metadata diagnostic 契约生成，记录 8 个 sex × ageBand 公开聚合域、minimum observed domain df = 15、0 个 lonely represented strata、0 个 empty domains 和 no-row/no-count/no-weighted-output 边界。
- `src/data/life-path-nhanes-public-lmf-disclosure-output-envelope-validation.json`：由 `npm run export:nhanes-disclosure-output-envelope-validation` 从 NHANES public-use LMF disclosure output envelope policy 和合成测试集生成，记录 8 个 synthetic allow/block 用例、2 allow / 6 block、policy/test-case hash、synthetic-only 和 no-real-output 边界。
- `src/data/life-path-nhanes-public-lmf-effective-sample-ci-publication-validation.json`：由 `npm run export:nhanes-effective-sample-ci-publication-validation` 从 NHANES public-use LMF effective sample / CI publication policy 和合成测试集生成，记录 9 个 synthetic allow/block 用例、2 allow / 7 block、policy/test-case hash、synthetic-only 和 no-real-output 边界。
- `src/data/life-path-nhanes-public-lmf-weighted-output-implementation-preflight-validation.json`：由 `npm run export:nhanes-weighted-output-implementation-preflight-validation` 从 NHANES public-use LMF weighted-output implementation preflight policy 和合成测试集生成，记录 8 个 synthetic allow/block 用例、2 allow / 6 block、policy/test-case hash、estimator-plan shape、synthetic-only 和 no-real-output 边界。
- `src/data/life-path-nhanes-public-lmf-disclosure-review-template-validation.json`：由 `npm run export:nhanes-disclosure-review-template-validation` 从 NHANES public-use LMF disclosure review template 生成，记录 15 个 pending review slots、template hash 和 no-real-output 边界；它只证明未来真实输出披露审查 packet 结构已登记，不证明真实 disclosure review 完成。
- `src/data/life-path-nhanes-public-lmf-weighted-domain-output-readiness-validation.json`：由 `npm run export:nhanes-weighted-domain-output-readiness-validation` 从 NHANES public-use LMF weighted-domain output readiness 契约生成，记录上游 controlled runtime smoke、domain indicator、DOF/sparse metadata diagnostic、synthetic disclosure envelope、synthetic publication criteria、synthetic implementation preflight、disclosure review template gates 和 no-weighted-domain-output 边界；它不进入默认 `audit:life-path-toy` 输出链，不证明真实 NHANES weighted-domain rates、design-based interval、校准或个体预测可用。
- `src/data/life-path-nhanes-public-lmf-public-output-implementation-review-template-validation.json`：由 `npm run export:nhanes-public-output-implementation-review-template-validation` 从 NHANES public-use LMF public-output implementation review template 生成，记录 12 个 pending implementation review slots、template hash 和 no-public-output 边界；它只证明未来公开前端实现审查 packet 结构已登记，不证明 public weighted-domain output 可以发布。
- `src/data/life-path-nhanes-public-lmf-public-output-implementation-review-execution-validation.json`：由 `npm run export:nhanes-public-output-implementation-review-execution-validation` 从 NHANES public-use LMF public-output implementation review execution register 生成，记录 12 个 implementation review execution slots 仍 pending、0 个 human-reviewed slots、无 reviewed artifact hash、无 second reviewer signoff 和 no-public-output 边界；它不证明 public weighted-domain output 可以发布。
- `src/data/life-path-nhanes-public-lmf-public-release-gate-validation.json`：由 `npm run export:nhanes-public-release-gate-validation` 从 NHANES public-use LMF disclosure execution、public Web no-real-values scan、local run evidence manifest、weighted-domain readiness、public-output implementation review template 和 public-output implementation review execution register 汇合生成，记录 release decision 仍为 blocked、5 个必要 release blockers 和 no-calibration / no-individual-prediction 边界；它不读取 ignored real output，不证明 public weighted-domain output 可发布。
- `src/data/life-path-nhats-disclosure-control-validation.json`：由 `npm run export:nhats-disclosure-validation` 从 NHATS disclosure policy 和 synthetic test cases 生成，记录 synthetic output envelope 的 allow/block 结果、policy hash、test-case hash 和 synthetic-only 边界。
- `src/data/life-path-nhats-survey-design-validation.json`：由 `npm run export:nhats-survey-design-validation` 从 NHATS survey-design protocol 和 synthetic test cases 生成，记录 synthetic design-plan envelope 的 allow/block 结果、protocol hash、test-case hash 和 synthetic-only 边界；它不证明真实 NHATS 加权估计、方差估计或人口推断可用。
- `src/data/life-path-nhats-missingness-route-validation.json`：由 `npm run export:nhats-missingness-route-validation` 从 NHATS missingness-route protocol 和 synthetic test cases 生成，记录 synthetic route envelope 的 allow/block 结果、protocol hash、test-case hash、route class 覆盖和 synthetic-only 边界；它不证明真实 NHATS route fields、endpoint classification、route counts 或死亡边界字段已确认。
- `src/data/life-path-nhats-route-field-discovery-validation.json`：由 `npm run export:nhats-route-field-discovery-validation` 从 NHATS route-field discovery register 生成，记录官方 R13/R14 crosswalk 字段发现、source evidence、field family、blocking gate、register hash 和禁止用途边界；它不证明 Colectica value labels、真实文件访问、route classifier、weighted route counts、public export、calibration 或 endpoint classification 可用。
- `src/data/life-path-nhats-colectica-value-label-validation.json`：由 `npm run export:nhats-colectica-value-label-validation` 从 NHATS Colectica value-label review protocol 生成，记录 source evidence、artifact requirements、route-field review units、blocking gates、protocol hash 和 no-confirmed-value-label-map 边界；它不证明真实 value labels、question text、skip logic、route classifier、weighted route counts、public export、calibration 或 endpoint classification 可用。
- `src/data/life-path-nhats-colectica-capture-packet-validator-validation.json`：由 `npm run export:nhats-colectica-capture-packet-validator-validation` 从 NHATS Colectica capture packet validator synthetic cases 生成，验证未来捕获包只能进入 `reviewable-but-still-blocked`、`cannot-evaluate` 或 `rejected`；它不证明真实登录、真实变量页捕获、人审完成、slot closure、route classifier、提取、公开导出、校准或个体预测可用。
- `src/data/life-path-nhats-colectica-capture-packet-review-execution-validation.json`：由 `npm run export:nhats-colectica-capture-packet-review-execution-validation` 从 NHATS Colectica capture-packet review execution register 生成，验证 39 个捕获包审查槽位仍为 pending、0 个真实包、0 个一审、0 个二审和 0 个关闭槽；它不证明真实登录、真实变量页捕获、人审完成、slot closure、route classifier、提取、公开导出、校准或个体预测可用。
- `src/data/life-path-nhats-route-value-crosswalk-assembly-validation.json`：由 `npm run export:nhats-route-value-crosswalk-assembly-validation` 从 NHATS route-value crosswalk assembly protocol 生成，验证 9 个 route-field assembly units、上游 hash、敏感 death-date 排除、0 个 reviewed capture packets、0 个 route-value rows、0 个变量级 missing-code maps 和 0 个模型准入；它不证明真实 crosswalk、真实分类器、提取、公开导出、校准或个体预测可用。
- `src/data/life-path-nhats-route-value-crosswalk-entry-validator-validation.json`：由 `npm run export:nhats-route-value-crosswalk-entry-validator-validation` 从 NHATS route-value crosswalk entry validator synthetic cases 生成，验证未来 route-value / missing-code 条目只能进入 `reviewable-but-still-blocked`、`cannot-evaluate` 或 `rejected`；它不证明真实 crosswalk 行、assembly unit closure、route classifier、提取、公开导出、校准或个体预测可用。
- `src/data/life-path-nhats-route-classifier-synthetic-dry-run-validation.json`：由 `npm run export:nhats-route-classifier-synthetic-dry-run-validation` 生成，先刷新 ignored crosswalk draft / review handoff，再把 8 个 synthetic route envelope 用例压缩成 tracked Web validation summary；它只证明分类器外壳的 allow/block 语义可运行，不读取或导出真实 NHATS 行、加权 route counts、公开输出、校准结果或个体预测。
- `src/data/life-path-nhats-official-source-refresh-validation.json`：由 `npm run export:nhats-official-source-refresh-validation` 从 NHATS official source refresh register 生成，验证 8 个官方公开来源、2026-07-04 live reprobe、gate impact 和 no-download / no-extraction / no-calibration / no-individual-prediction 边界；它不证明 HTML 语义稳定、注册完成、下载许可、提取许可、校准或个体预测可用。
- `src/data/life-path-nhats-registration-evidence-template-validation.json`：由 `npm run export:nhats-registration-evidence-template-validation` 从 NHATS registration evidence template 生成，验证未来注册证据包必须提供的 evidence slots、人工复核槽和 no-download / no-extraction / no-calibration / no-individual-prediction 边界；它不证明真实注册完成、账号许可、下载许可、提取许可、校准或个体预测可用。
- `src/data/life-path-nhats-registration-evidence-packet-validator-validation.json`：由 `npm run export:nhats-registration-evidence-packet-validator-validation` 从 NHATS registration evidence packet validator synthetic cases 生成，验证未来注册证据包只能进入 `reviewable-but-still-blocked`、`cannot-evaluate` 或 `rejected`；它不接受凭证、token、原始行级数据、个体死亡日期或公开 AI 上传，也不证明真实注册、下载、提取、校准或个体预测可用。
- `src/data/life-path-nhats-l2-variable-family-admission-validation.json`：由 `npm run export:nhats-l2-variable-family-admission-validation` 生成，并由 `npm run audit:life-path-toy` 自动刷新；它校验 NHATS L2 variable-family admission register 对 first estimand、变量确认矩阵、模型准入契约、准入候选注册表和 authenticated capture template 的哈希绑定，只证明 L2 候选族映射，不证明 L4 校准、真实抽取、真实验证、个体预测或医疗建议。
- `src/data/life-path-toy-model-audit.json` / `src/data/life-path-toy-model-audit.md`：由 `npm run audit:life-path-toy` 生成的模型审计产物，记录 schema、source hash、model card、曲线、概率范围、LEV 开放边界、合成敏感性分析、NHANES public-use LMF survey-design readiness、NHANES domain/subpopulation rule readiness、NHANES positive-weight eligible-base readiness、NHANES weighted-estimator readiness、NHANES R survey runtime smoke readiness、NHATS official-source-refresh validation、NHATS disclosure validation、NHATS survey-design validation、NHATS missingness-route validation、NHATS route-field discovery validation、NHATS Colectica value-label validation、NHATS Colectica access-route probe validation、NHATS Colectica authenticated capture template validation、NHATS Colectica capture packet validator/review execution validation、NHATS route-value crosswalk assembly validation、NHATS route-value crosswalk entry validator validation、校准预备契约、候选数据源注册表、数据源 Source Cards、Data Card 模板、NHATS Data Card、NHATS 变量字典、NHATS extraction manifest、NHATS 机器可读 acquisition-readiness gates、NHATS file-tier table、NHATS first estimand protocol、NHATS variable confirmation matrix、NHATS cohort-flow endpoint-routing protocol 和禁止个体死亡日期字段的检查结果。
- `src/data/life-path-toy-model-audit.json` / `.md` 现在还包含 `nhatsRegistrationEvidencePacketValidator` 节点，消费 NHATS registration evidence template、registration evidence packet validator synthetic cases 和 validation JSON；该节点只证明未来注册证据包预检语义被统一审计，仍保持真实注册、下载、提取、校准、公开导出和个体预测全部阻塞。
- `src/data/life-path-toy-model-audit.json` / `.md` 现在还包含 `nhatsRouteClassifierSyntheticDryRun` 节点，消费 tracked NHATS route-classifier synthetic dry-run validation summary；该节点只证明 8 个 synthetic route envelope 用例的 allow/block 分类外壳被统一审计，真实 route classifier、真实提取、加权统计、公开导出、校准和个体预测仍全部阻塞。
- `src/data/life-path-toy-model-audit.json` / `.md` 现在还包含 `l4ControlPlane` 节点，消费 L4 阻塞矩阵、解阻执行计划、证据 intake register、验证/校准报告契约、执行寄存器和执行验证 JSON；该节点只证明 L4 阻塞状态被统一审计，仍保留 24 个 pending evidence slots、0 个真实报告包、0 个人审签名、`l4-still-blocked`、无公开加权输出、无校准预测和无个体预测。
- `src/data/life-path-toy-model-audit.json` / `.md` 现在还包含 `modelGovernanceControlPlane` 节点，消费模型准入契约、准入候选注册表、定量能力分层、能力分层 validation、科研标准锚点注册表、科研标准 validation、域到模型桥接契约、桥接注册表和桥接 validation；该节点只证明这些治理控制面被统一审计，仍保持 Q3/L3 为当前能力上限，将 C1-C6 域桥接限制在 Q2/L2 候选变量和模型位置说明，并阻塞系数、因果效应、Q4 校准聚合预测、Q5 个体用途、死亡日期输出、医疗建议和干预排序。
