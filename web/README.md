...
- `src/data/life-path-toy-model-audit.json` / `src/data/life-path-toy-model-audit.md`：由 `npm run audit:life-path-toy` 生成的模型审计产物，记录 schema、source hash、model card、曲线、概率范围、LEV 开放边界、校准预备契约、候选数据源注册表、数据源 Source Cards、Data Card 模板和禁止个体死亡日期字段的检查结果。
