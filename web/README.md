...
- `src/data/life-path-nhanes-public-lmf-weighted-domain-output-readiness-validation.json`：由 `npm run export:nhanes-weighted-domain-output-readiness-validation` 从 NHANES public-use LMF weighted-domain output readiness 契约生成，记录上游 controlled runtime smoke、domain indicator、DOF/sparse-domain、disclosure gates 和 no-weighted-domain-output 边界；它不进入默认 `audit:life-path-toy` 输出链，不证明真实 NHANES weighted-domain rates、design-based interval、校准或个体预测可用。
