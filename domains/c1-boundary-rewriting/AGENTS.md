...
- 奇点相关域按边界类型拆分为超人智能阈值、递归自我改进、加速回报收敛、控制/解释权边界；不要重新合并成单一“singularity”大桶。
- `disembodied-cns/` 的当前核心不是器官形态复制，而是脑-身接口协议、接口因果等价和最小充分身体；维护时不得把局部器官支持或设备相似性写成主体连续性成立。
...
- 新增 C1 研究域时必须同步更新本文件的目录结构和 `README.md` 的当前研究域列表。
