...
- `../../../../web/src/data/life-path-nhanes-public-lmf-weighted-domain-output-readiness-validation.json`：由 `validate_nhanes_public_lmf_weighted_domain_output_readiness.py` 生成的 weighted-domain output readiness validation 输出，记录 5 个 ready gates、4 个 blocked gates、domain indicator / DOF / disclosure 前置条件和 no-weighted-domain-output 边界。
