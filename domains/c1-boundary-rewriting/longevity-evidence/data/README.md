...
- `manual/life_path_nhanes_public_lmf_weighted_domain_output_readiness.json` 用于维护 NHANES public-use LMF weighted-domain output 前的安全门契约，绑定 controlled R `survey` smoke、post-design domain indicator、DOF/sparse-domain review 和 disclosure review；它只登记输出前置条件，不授权 weighted domain rates、design-based intervals、校准或个体预测。
...
- `web/src/data/life-path-nhanes-public-lmf-weighted-domain-output-readiness-validation.json` 是从 NHANES public-use LMF weighted-domain output readiness 契约派生的验证报告；它不放在本目录内，但由本域脚本生成并由本域审计器检查，只证明输出安全门已登记、controlled runtime smoke 与 public output permission 已分离，并继续禁止真实 weighted domain output、置信区间、校准或个体预测。
...
- `manual/life_path_nhanes_public_lmf_weighted_domain_output_readiness.json`：NHANES public-use LMF weighted-domain output readiness 契约，记录 controlled runtime smoke 上游绑定、domain indicator、DOF/sparse-domain、disclosure 和继续阻塞的 public output 边界。
...
- `../../../../web/src/data/life-path-nhanes-public-lmf-weighted-domain-output-readiness-validation.json`：由 `validate_nhanes_public_lmf_weighted_domain_output_readiness.py` 生成的 weighted-domain output readiness validation 输出，记录 6 个 ready gates、3 个 blocked gates、domain indicator metadata diagnostic、DOF / disclosure 前置条件和 no-weighted-domain-output 边界。
