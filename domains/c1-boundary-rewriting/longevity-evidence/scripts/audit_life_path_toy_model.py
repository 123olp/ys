...
DEFAULT_NHANES_PUBLIC_LMF_ELIGIBLE_BASE_READINESS = (
    REPO_ROOT
    / "domains"
    / "c1-boundary-rewriting"
    / "longevity-evidence"
...
DEFAULT_NHANES_PUBLIC_LMF_ELIGIBLE_BASE_READINESS_VALIDATION = (
    REPO_ROOT
    / "web"
    / "src"
    / "data"
...
def audit_nhanes_public_lmf_eligible_base_readiness(
    readiness_path: Path,
    validation_path: Path,
) -> dict[str, Any]:
    checks: list[dict[str, Any]] = []
    ...
    add_check(
        checks,
        "nhanes-public-lmf-eligible-base-readiness-exists",
        status_from_bool(readiness_exists),
        str(readiness_path.relative_to(REPO_ROOT)),
    )
    add_check(
        checks,
        "nhanes-public-lmf-eligible-base-readiness-validation-exists",
        status_from_bool(validation_exists),
        str(validation_path.relative_to(REPO_ROOT)),
    )
    ...
    add_check(
        checks,
        "nhanes-public-lmf-eligible-base-readiness-schema",
        status_from_bool(readiness_exists and schema_ok),
        f"schemaVersion={readiness.get('schemaVersion')!r}",
    )

    rule = readiness.get("eligibleBaseRule")
    rule_ok = (
        isinstance(rule, dict)
        and rule.get("analysisWeight") == "WTMEC2YR"
        and rule.get("positiveWeightCondition") == "WTMEC2YR > 0"
        and rule.get("fullDesignInputBeforeDomainRequired") is True
        and rule.get("subpopulationIndicatorRequiredAfterEligibleBase") is True
        and rule.get("rowDropBeforeDesignAllowed") is False
        and rule.get("eligibleBaseRowsPersisted") is False
        and rule.get("weightedEstimatorImplemented") is False
        and rule.get("weightedDomainInferenceAllowed") is False
        and rule.get("designBasedIntervalAllowed") is False
        and rule.get("calibrationAllowed") is False
        and rule.get("individualPredictionAllowed") is False
        and rule.get("medicalAdviceAllowed") is False
    )
    add_check(
        checks,
        "nhanes-public-lmf-eligible-base-rule-boundary",
        status_from_bool(readiness_exists and rule_ok),
        "eligible-base rule must require WTMEC2YR > 0, full design input, no row drop and no weighted inference",
    )

    diagnostics = readiness.get("aggregateDiagnostics")
    diagnostics_ok = (
        isinstance(diagnostics, dict)
        and diagnostics.get("eligibleAdultRecordsInTemp") == 5809
        and diagnostics.get("eligibleAdultDeaths") == 145
        and diagnostics.get("positiveMecWeightEligibleAdultRecordsInTemp") == 5809
        and diagnostics.get("zeroMecWeightEligibleAdultRecordsInTemp") == 0
        and diagnostics.get("missingMecWeightEligibleAdultRecordsInTemp") == 0
        and diagnostics.get("positiveMecWeightEligibleAdultDeaths") == 145
        and diagnostics.get("positiveWeightStrataInTemp") == 15
        and diagnostics.get("minimumPsuPerPositiveWeightStratumInTemp") == 2
        and diagnostics.get("lonelyPositiveWeightStrataInTemp") == 0
    )
    add_check(
        checks,
        "nhanes-public-lmf-eligible-base-diagnostics",
        status_from_bool(readiness_exists and diagnostics_ok),
        "positive-weight diagnostics must preserve 5809 eligible adults, 145 deaths, 15 strata and no lonely strata",
    )
    ...
    gate_summary_ok = (
        isinstance(gate_summary, dict)
        and gate_summary.get("requiredGateCount") == 8
        and gate_summary.get("readyGateCount") == 4
        and gate_summary.get("partialGateCount") == 0
        and gate_summary.get("blockedGateCount") == 4
        and gate_summary.get("weightedDomainInferenceAllowed") is False
    )
    add_check(
        checks,
        "nhanes-public-lmf-eligible-base-gate-summary",
        status_from_bool(readiness_exists and gate_summary_ok),
        "gate summary must keep weighted domain inference blocked despite positive-weight eligible-base diagnostics",
    )
    ...
    add_check(
        checks,
        "nhanes-public-lmf-eligible-base-source-findings",
        status_from_bool(readiness_exists and findings_ok),
        "source findings must include DEMO_J WTMEC2YR distribution, non-zero design input and no row-drop guidance",
    )
    ...
    add_check(
        checks,
        "nhanes-public-lmf-eligible-base-validation-source-hash",
        status_from_bool(validation_exists and validation_schema_ok and source_ok),
        "validation must point back to current eligible-base readiness path and sha256",
    )

    summary = validation.get("summary")
    validation_summary_ok = (
        isinstance(summary, dict)
        and summary.get("positiveMecWeightEligibleAdultRecords") == 5809
        and summary.get("zeroMecWeightEligibleAdultRecords") == 0
        and summary.get("weightedDomainInferenceAllowed") is False
    )
    add_check(
        checks,
        "nhanes-public-lmf-eligible-base-validation-summary",
        status_from_bool(validation_exists and validation_summary_ok),
        "validation summary must expose positive-weight eligible count while keeping weighted inference blocked",
    )
...
def audit_model(
    ...
    nhanes_public_lmf_eligible_base_readiness_audit = (
        audit_nhanes_public_lmf_eligible_base_readiness(
...
def render_markdown(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            f"- Readiness path: `{audit['nhanesPublicLmfEligibleBaseReadiness']['readinessPath']}`",
            f"- Readiness SHA-256: `{audit['nhanesPublicLmfEligibleBaseReadiness']['readinessSha256']}`",
            f"- Validation path: `{audit['nhanesPublicLmfEligibleBaseReadiness']['validationPath']}`",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--nhanes-public-lmf-eligible-base-readiness",
        type=Path,
        default=DEFAULT_NHANES_PUBLIC_LMF_ELIGIBLE_BASE_READINESS,
    )
    parser.add_argument(
        "--nhanes-public-lmf-eligible-base-readiness-validation",
        type=Path,
...
def main() -> int:
    ...
    nhanes_public_lmf_eligible_base_readiness_path = (
        args.nhanes_public_lmf_eligible_base_readiness.resolve()
    )
    nhanes_public_lmf_eligible_base_readiness_validation_path = (
