# accelerating-returns-technology-convergence 目录说明
...
`accelerating-returns-technology-convergence/` 维护加速回报与多技术收敛研究域。它把技术收敛写成可审查的窗口函数，而不是未来学口号。
...
- 每条“加速”主张必须拆成发现、验证、转化、部署、可及性和治理六个门槛。
- 不用单领域指数曲线替代全链路技术可用性。
- 与 `longevity-evidence/` 交叉时，只保存 C1 技术窗口边界模型；具体干预证据留在长寿证据域。
