...
10. 语言门户必须生成 Pages 原生 `404.html`；未知路径必须返回 HTTP 404，不得以首页 HTML 和 200 形成软 404。
11. Pages Worker 只负责快照路由、模板注入和只读标题搜索，不承载内容编辑、医学模型或数据库。
12. 远端 smoke 全部通过后，才能把发布视为完成。
