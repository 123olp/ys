...
| `timeline.json` | 正式年表数据：2584 条事件 |
| `sources.json` | 正式来源卡：2594 个来源 |
...
- 来源卡：2594 个来源已注册，包含重复引用和疑似错源标注。
- 事件：2584 条事件已转为结构化 JSON，覆盖神话、思想、技术、制度、实践和失败教训。
...
- 质量：日期粒度已按 `exact` / `approx` 契约修正；同源重复已合并 8 组，剩余 1 组疑似错源待复核。
