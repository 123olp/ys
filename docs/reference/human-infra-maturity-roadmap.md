...
| 研究框架 | 83% | 每条主张都进入 Source Card、Claim-Evidence Matrix、变量表和反证条件 | 已有核心主张矩阵、页面级 Claim ID 门禁、arXiv-style 论文页强主张门禁、C1/20 个优先 C2 反证覆盖门禁、v0.1 反证 Source Card 锚点回填、当前 21 个来源锚点字段级 Source Card 抽取、26 个优先域的域级 Claim-Evidence Matrix seed、26 个优先域的 endpoint 候选和 source-specific 深读槽位、方法锚点和本地审计门禁，但 20 域之外的 C2 长尾域、source-specific endpoint / population / uncertainty 精读和外部文献核验仍未完成 |
...
- 把 `human-infra-domain-source-card-field-extraction.json` 的 26 条字段行继续拆成 source-specific endpoint / population / effect-size / uncertainty / transfer-boundary Source Cards，并继续把 C1/C2 长尾域接入同一矩阵。
