...
- 按 `human-infra-c2-longtail-first-batch-promotion-queue.json` 先执行 C2-LT-B1：把 24 个高影响未覆盖 C2 长尾域推进到 Claim-Evidence row、source extraction、source-specific deep read、local source-context review、independent fresh review 和 reviewed artifacts。所有模型门禁完成前仍禁止模型准入。
