...
| 定量模型 | 87% | 有可运行、可复现、可审查的场景级模型管线 | 已有 toy model、合成敏感性分析、审计器、NCHS 2021 U.S. Life Tables 公开聚合死亡率锚点、NHANES public-use LMF aggregate pilot、NHANES public-use LMF survey-design readiness validator、NHANES public-use LMF domain/subpopulation rule readiness validator、NHANES public-use LMF positive-weight eligible-base readiness validator、NHANES public-use LMF weighted-estimator readiness validator、NHANES public-use LMF R survey runtime smoke validator、controlled R survey runtime smoke proof、校准预备契约、真实队列候选注册表、NHATS 数据准入草案、变量字典草案、extraction manifest 草案、official source refresh register / validator、registration evidence template / validator、R13/R14 file-tier table / validator、acquisition-readiness gates / validator、第一版 estimand protocol、变量确认矩阵、cohort-flow endpoint protocol、disclosure / survey-design / missingness / route-field / Colectica value-label protocol validator / Colectica value-label execution validator / Colectica access-route probe validator / Colectica authenticated capture template validator / L2 variable-family admission validator / pre-outcome aggregation validator / L4 readiness runway validator、模型准入契约和模型准入候选注册表；但 NHANES survey-design readiness 只登记 WTMEC2YR、SDMVPSU、SDMVSTRA 和 no-weighted-inference 边界，domain/subpopulation rule readiness 只登记官方 domain 机制、完整设计输入要求和禁止 row-drop subgroup filtering 的边界，eligible-base readiness 只登记 `WTMEC2YR > 0` 正权重诊断和 no-row-persistence 边界，weighted-estimator readiness 只选择 R `survey` / `svydesign` 后端并绑定 design-object / domain indicator 合约，controlled runtime smoke 只证明 synthetic R `survey` 运行时可复现，NHANES 试运行仍只是公开聚合烟测，还没有真实 NHATS 注册证明、Colectica 登录、变量页实捕获、value labels 精确确认、question text / universe / skip logic 确认、真实提取、真实 NHATS route classification、真实 NHATS 输出披露审查、真实 survey-design 加权估计、真实 NHANES weighted domain output、外部验证、校准后的敏感性分析和任何 L4 calibrated admission |
