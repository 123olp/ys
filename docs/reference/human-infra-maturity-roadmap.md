...
- 按 `human-infra-domain-source-card-promotion-queue.json` 执行 81 个晋升任务，把 completed rows 推进到 fresh-reviewed Source Cards、变量卡、endpoint 卡、uncertainty 卡、transfer-boundary 卡和 downgrade checks，并继续把 C1/C2 长尾域接入同一矩阵。
