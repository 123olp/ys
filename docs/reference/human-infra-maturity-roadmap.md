...
| 研究框架 | 73% | 每条主张都进入 Source Card、Claim-Evidence Matrix、变量表和反证条件 | 已有核心主张矩阵、页面级 Claim ID 门禁、arXiv-style 论文页强主张门禁、C1/20 个优先 C2 反证覆盖门禁、v0.1 反证 Source Card 锚点回填、方法锚点和本地审计门禁，但 20 域之外的 C2 长尾域和逐篇 Source Card 精读仍未完成 |
...
- 把 `human-infra-falsifier-source-card-backfill.json` 中的 v0.1 来源锚点继续拆成逐篇 Source Cards，并把 C1/C2 核心主张做成 Claim-Evidence Matrix。
