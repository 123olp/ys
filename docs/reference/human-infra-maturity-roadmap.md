...
- 按 `human-infra-reviewed-card-artifact-register.json` 和 `human-infra-future-boundary-route-card-register.json` 从 486 个 reviewed card artifacts 与 4 条 future-boundary route cards 出发，把同一 Source/variable/endpoint/uncertainty/transfer/downgrade 契约推广到 C2 长尾域。所有模型门禁完成前仍禁止模型准入。
