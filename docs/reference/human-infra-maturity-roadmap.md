...
- 按 `human-infra-c2-longtail-first-batch-source-extraction-queue.json` 执行 C2-LT-B1 的 48 个 source-specific 深读任务：先填 exact claim、endpoint、population/setting、mechanism/effect、uncertainty、transfer boundary、downgrade trigger 和 model position，再进入 local source-context review、independent fresh review 和 reviewed artifacts。所有模型门禁完成前仍禁止模型准入。
