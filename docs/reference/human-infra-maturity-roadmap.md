...
- 按 `human-infra-c2-longtail-first-batch-source-extraction-queue.json` 执行 C2-LT-B1 的 48 个 source-specific 深读任务：当前 `human-infra-c2-longtail-first-batch-source-extraction-register.json` 已完成 48/48 个来源语境字段抽取，`human-infra-c2-longtail-first-batch-local-review-register.json` 已完成 48/48 个本地来源语境复核，`human-infra-c2-longtail-first-batch-independent-fresh-review-protocol.json` 已覆盖 48/48 个 fresh-review 批次，`human-infra-c2-longtail-first-batch-independent-fresh-review-verdict-register.json` 已完成四批 48/48 个 fresh-review verdict，`human-infra-c2-longtail-first-batch-reviewed-card-artifact-register.json` 已把 42 个 eligible rows 晋升为 252 个 reviewed artifacts，`human-infra-c2-longtail-first-batch-blocked-source-resolution-register.json` 已为 6 个非 eligible rows 准备来源校正候选，`human-infra-c2-longtail-first-batch-source-resolution-fresh-review-verdict-register.json` 已完成这 6 行的来源纠偏 fresh review 判定，`human-infra-c2-longtail-first-batch-corrected-source-reextraction-queue.json` 已把 10 个 selected corrected candidates 派生成重新抽取任务，`human-infra-c2-longtail-first-batch-corrected-source-reextraction-register.json` 已完成 10/10 个 bounded corrected extraction 输出，`human-infra-c2-longtail-first-batch-corrected-source-fresh-review-verdict-register.json` 已完成 10/10 个 corrected extraction outputs 的 independent fresh review 判定，`human-infra-c2-longtail-first-batch-corrected-source-reviewed-card-artifact-register.json` 已把 5 个 eligible corrected rows 晋升为 30 个 bounded reviewed artifacts；`human-infra-c2-longtail-second-batch-promotion-queue.json` 已选择 12 个剩余 C2 长尾域并绑定 24 个 web-checked 候选来源，`human-infra-c2-longtail-second-batch-source-extraction-queue.json` 已把它们派生成 24 个 source-specific 深读任务，`human-infra-c2-longtail-second-batch-source-extraction-register.json` 已完成 C2-LT-B2 24/24 个来源语境字段抽取，`human-infra-c2-longtail-second-batch-local-review-register.json` 已完成 24/24 个本地来源语境复核，`human-infra-c2-longtail-second-batch-independent-fresh-review-protocol.json` 已把 24 行拆成 2 个 fresh-review 批次，`human-infra-c2-longtail-second-batch-independent-fresh-review-verdict-register.json` 已完成 24/24 个 fresh-review verdict，其中 23 行只允许 bounded artifact fill，1 行必须 downgrade-before-fill，`human-infra-c2-longtail-second-batch-reviewed-card-artifact-register.json` 已把 23 个 eligible rows 晋升为 138 个 bounded reviewed artifacts；1 个 downgrade-before-fill 行和 5 个 lineage/route/index/fulltext 阻塞行继续保留边界。`human-infra-c2-longtail-third-batch-promotion-queue.json` 已选择 12 个神经-感知-认知连续性域并绑定 24 个 web-checked 候选来源，`human-infra-c2-longtail-third-batch-source-extraction-queue.json` 已把它们派生成 24 个 source-specific 深读任务，`human-infra-c2-longtail-third-batch-source-extraction-register.json` 已完成 24/24 个来源语境字段抽取，`human-infra-c2-longtail-third-batch-local-review-register.json` 已完成 24/24 个本地来源语境复核并保留 5 个 source-resolution/manual-access 问题行，`human-infra-c2-longtail-third-batch-source-resolution-register.json` 已把这 5 行准备为 7 个 corrected/split/route-normalized 候选；下一步执行 C2-LT-B3 source-resolution fresh review、corrected source re-extraction 和 bounded artifact fill，并在所有模型门禁完成前保持模型准入禁止。
