...
- 把 NHATS manifest、controlled storage/destruction plan、route-field discovery register、Colectica value-label review protocol、Colectica value-label review execution register、Colectica access-route probe register、Colectica authenticated capture template、L2 variable-family admission register、pre-outcome aggregation protocol 和 first estimand protocol 从 draft / partial-executed / login-required / template-only / L2-only / protocol-only 推进到 governed acquisition-ready，补真实受控工作区执行记录、access/inventory log、受控工作区内销毁证据、Colectica 登录复核、variable page capture hash、value labels、question text、universe/skip logic、精确字段名、轮次、变量级缺失码、公开/敏感/受限状态、权重、endpoint 定义、cohort flow、survey design、代码本来源、真实聚合前置证据和输出抑制规则。
