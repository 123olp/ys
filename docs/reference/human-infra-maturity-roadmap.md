...
- 按 `human-infra-c2-longtail-first-batch-source-extraction-queue.json` 执行 C2-LT-B1 的 48 个 source-specific 深读任务：当前 `human-infra-c2-longtail-first-batch-source-extraction-register.json` 已完成前 12/48 个来源语境字段抽取；下一步补齐剩余 36 个来源，再进入 local source-context review、independent fresh review 和 reviewed artifacts。所有模型门禁完成前仍禁止模型准入。
