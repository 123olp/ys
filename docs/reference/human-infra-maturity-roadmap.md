...
- 按 `human-infra-reviewed-card-artifact-register.json` 从 486 个 reviewed card artifacts 出发，继续把高优先 future-boundary 路线补成 route cards，并把同一 Source/variable/endpoint/uncertainty/transfer/downgrade 契约推广到 C2 长尾域。所有模型门禁完成前仍禁止模型准入。
