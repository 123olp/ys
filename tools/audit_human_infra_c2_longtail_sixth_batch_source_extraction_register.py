...
REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-sixth-batch-source-extraction-register.json"
QUEUE_PATH = ROOT / "docs/reference/human-infra-c2-longtail-sixth-batch-source-extraction-queue.json"

SCHEMA = "human-infra.c2-longtail-sixth-batch-source-extraction-register.v1"
STATUS = "active-c2ltb6-source-extractions-pending-fresh-review"
REGISTER_LINK = "human-infra-c2-longtail-sixth-batch-source-extraction-register.json"
...
def validate_scope(scope: Any, rows: list[dict[str, Any]], queue_count: int, errors: list[str]) -> None:
    ...
    if selected != EXPECTED_TASK_IDS:
        fail(errors, "scope.selectedTaskIds must equal all 24 C2LTB6 extraction task IDs")
...
def queue_tasks(errors: list[str]) -> dict[str, dict[str, Any]]:
    queue = load_json(QUEUE_PATH, errors, "C2 sixth-batch source extraction queue")
...
def validate_rows(rows: Any, task_map: dict[str, dict[str, Any]], errors: list[str]) -> list[dict[str, Any]]:
    ...
    if [row.get("taskId") for row in result] != EXPECTED_TASK_IDS:
        fail(errors, "extractedRows must be ordered as C2LTB6-EXT-001 through C2LTB6-EXT-024")
    return result
...
def main() -> int:
    errors: list[str] = []
    data = load_json(REGISTER_PATH, errors, "C2 sixth-batch source extraction register")
    ...
    print(
        "C2 longtail sixth-batch source extraction register audit ok: "
