#!/usr/bin/env python3
"""审计 C2-LT-B5 source-resolution 账本。"""
...
REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-fifth-batch-source-resolution-register.json"
QUEUE_PATH = ROOT / "docs/reference/human-infra-c2-longtail-fifth-batch-source-extraction-queue.json"
EXTRACTION_PATH = ROOT / "docs/reference/human-infra-c2-longtail-fifth-batch-source-extraction-register.json"
LOCAL_REVIEW_PATH = ROOT / "docs/reference/human-infra-c2-longtail-fifth-batch-local-review-register.json"

SCHEMA = "human-infra.c2-longtail-fifth-batch-source-resolution-register.v1"
STATUS = "active-c2ltb5-source-resolution-register-independent-fresh-review-required"
REGISTER_LINK = "human-infra-c2-longtail-fifth-batch-source-resolution-register.json"
SCRIPT_LINK = "audit_human_infra_c2_longtail_fifth_batch_source_resolution_register.py"
...
EXPECTED_VERIFICATION_IDS = {
    "C2LTB5-SR-VERIFY-NCBI-PUBMED-20260703",
    "C2LTB5-SR-VERIFY-EUROPEPMC-ROUTE-20260703",
    "C2LTB5-SR-VERIFY-PMC-SENESCENCE-FULLTEXT-20260703",
}
...
def validate_scope(data: dict[str, Any], rows: list[dict[str, Any]], errors: list[str]) -> None:
    ...
    expected = {
        ...
        "duplicateCrossDomainIssueCount": len(DUPLICATE_SOURCE_TASK_IDS),
        "noOpenFulltextIssueCount": len(NO_OPEN_FULLTEXT_TASK_IDS),
        "resolutionCandidateCount": EXPECTED_CANDIDATE_COUNT,
    }
    if scope.get("batchId") != "C2-LT-B5":
        fail(errors, "scope.batchId must be C2-LT-B5")
    if scope.get("resolutionLevel") != "c2ltb5-local-review-issue-row-source-resolution-v0.1":
...
def validate_row(
    ...
    if task_id in NO_OPEN_FULLTEXT_TASK_IDS and "no-open-fulltext" not in source_context:
        fail(errors, f"{task_id} extraction row must preserve no-open-fulltext source issue status")
    if task_id in DUPLICATE_SOURCE_TASK_IDS and not any(
        marker in source_context for marker in ["duplicate", "shared-source", "shared_source"]
    ):
        fail(errors, f"{task_id} extraction row must preserve duplicate or shared-source issue status")
...
def validate_rows(
    ...
    if [row.get("taskId") for row in rows if isinstance(row, dict)] != EXPECTED_TASK_IDS:
        fail(errors, "resolutionRows must cover expected C2-LT-B5 issue task IDs in order")
...
def main() -> int:
    errors: list[str] = []
    data = load_json(REGISTER_PATH, errors, "C2-LT-B5 source-resolution register")
    ...
    if errors:
        ...
        return 1
    print("C2-LT-B5 source-resolution register audit ok: rows=8 candidates=14 model=blocked")
    return 0
