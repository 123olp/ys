...
QUEUE_PATH = ROOT / "docs/reference/human-infra-c2-longtail-eleventh-batch-promotion-queue.json"
...
SCHEMA = "human-infra.c2-longtail-eleventh-batch-promotion-queue.v1"
STATUS = "active-eleventh-batch-queue-model-blocked"
BATCH_ID = "C2-LT-B11"
EXPECTED_ROWS = 12
EXPECTED_SOURCES = 24
REGISTER_LINK = "human-infra-c2-longtail-eleventh-batch-promotion-queue.json"
SCRIPT_LINK = "audit_human_infra_c2_longtail_eleventh_batch_promotion_queue.py"
...
REQUIRED_INDEX_LINKS = {
    ...
    "Makefile": "c2-longtail-eleventh-batch-promotion-audit",
...
def validate_candidate_source(ref: Any, row_index: int, source_index: int, seen_refs: set[str], errors: list[str]) -> None:
    ...
    if source_id:
        ...
        if not source_id.startswith("C2LTB11-SRC-"):
            fail(errors, f"{context}.sourceRefId must start with C2LTB11-SRC-")
...
def main() -> int:
    errors: list[str] = []
    data = load_json(QUEUE_PATH, errors, "C2-LT-B11 promotion queue")
    ...
    if data:
        ...
        if not isinstance(source, dict) or "tenthBatchPromotionQueue" not in source:
            fail(errors, "sourceOfTruth must include previous B1-B10 queues")
    ...
    if errors:
        ...
        return 1
    print(f"C2-LT-B11 promotion queue audit ok: domains={EXPECTED_ROWS} sources={EXPECTED_SOURCES}")
    return 0
