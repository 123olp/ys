#!/usr/bin/env python3
"""审计 C2-LT-B2 reviewed card artifact 账本。"""
...
REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-second-batch-reviewed-card-artifact-register.json"
VERDICT_PATH = ROOT / "docs/reference/human-infra-c2-longtail-second-batch-independent-fresh-review-verdict-register.json"
...
SCHEMA = "human-infra.c2ltb2-reviewed-card-artifact-register.v1"
STATUS = "active-c2ltb2-reviewed-card-artifact-register-model-blocked"
REGISTER_LINK = "human-infra-c2-longtail-second-batch-reviewed-card-artifact-register.json"
...
def validate_scope(
    ...
    if not isinstance(scope, dict):
        ...
        return set(), set()
    if scope.get("batchId") != "C2-LT-B2":
        fail(errors, "scope.batchId must be C2-LT-B2")
    if scope.get("artifactLevel") != "c2ltb2-fresh-review-verdict-row-to-reviewed-card-artifact":
    ...
    for phrase in ["not model-ready", "does not authorize individual advice", "does not fill rows", "does not mark remaining C2"]:
...
def validate_register(errors: list[str]) -> tuple[int, int]:
    register = load_json(REGISTER_PATH, errors, "C2-LT-B2 reviewed card artifact register")
    verdict = load_json(VERDICT_PATH, errors, "C2-LT-B2 independent fresh-review verdict register")
    extraction_by_task = load_rows(EXTRACTION_PATH, "extractedRows", errors, "source extraction register")
    queue = load_json(QUEUE_PATH, errors, "C2-LT-B2 promotion queue")
    if not register:
        return 0, 0
    ...
    if not isinstance(reviews, list) or len(reviews) != 24:
        fail(errors, "fresh-review verdict register must contain 24 sourceTaskFreshReviews")
        reviews = []
    ...
    if queue and queue.get("scope", {}).get("batchId") != "C2-LT-B2":
        fail(errors, "promotion queue scope.batchId must be C2-LT-B2")
...
def main() -> int:
    ...
    if errors:
        print("C2-LT-B2 reviewed card artifact audit failed:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    print(f"C2-LT-B2 reviewed card artifact audit ok: artifacts={artifact_count} task_type_pairs={pair_count}")
    return 0
