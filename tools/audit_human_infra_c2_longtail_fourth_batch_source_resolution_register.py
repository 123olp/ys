#!/usr/bin/env python3
"""审计 C2-LT-B4 source-resolution 账本。"""
...
REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-fourth-batch-source-resolution-register.json"
QUEUE_PATH = ROOT / "docs/reference/human-infra-c2-longtail-fourth-batch-source-extraction-queue.json"
EXTRACTION_PATH = ROOT / "docs/reference/human-infra-c2-longtail-fourth-batch-source-extraction-register.json"
LOCAL_REVIEW_PATH = ROOT / "docs/reference/human-infra-c2-longtail-fourth-batch-local-review-register.json"

SCHEMA = "human-infra.c2-longtail-fourth-batch-source-resolution-register.v1"
STATUS = "active-c2ltb4-source-resolution-register-independent-fresh-review-required"
REGISTER_LINK = "human-infra-c2-longtail-fourth-batch-source-resolution-register.json"
SCRIPT_LINK = "audit_human_infra_c2_longtail_fourth_batch_source_resolution_register.py"
...
NO_ABSTRACT_TASK_IDS = {"C2LTB4-EXT-005", "C2LTB4-EXT-011", "C2LTB4-EXT-020"}
EXPECTED_CANDIDATE_COUNT = 8
EXPECTED_VERIFICATION_IDS = {
    "C2LTB4-SR-VERIFY-NCBI-PUBMED-20260703",
    "C2LTB4-SR-VERIFY-DIABETES-PMC-DUPLICATE-20260703",
    "C2LTB4-SR-VERIFY-ESC-GUIDELINE-PAGE-20260703",
    "C2LTB4-SR-VERIFY-AASLD-PMC-PUBMED-20260703",
    "C2LTB4-SR-VERIFY-AASLD-PAGE-20260703",
    "C2LTB4-SR-VERIFY-AGA-GUIDANCE-PAGE-20260703",
}
...
def validate_scope(data: dict[str, Any], rows: list[dict[str, Any]], errors: list[str]) -> None:
    ...
    expected = {
        ...
        "duplicateLineageIssueCount": 1,
        "noAbstractFulltextIssueCount": len(NO_ABSTRACT_TASK_IDS),
        "resolutionCandidateCount": EXPECTED_CANDIDATE_COUNT,
    }
    if scope.get("batchId") != "C2-LT-B4":
        fail(errors, "scope.batchId must be C2-LT-B4")
    if scope.get("resolutionLevel") != "c2ltb4-local-review-issue-row-source-resolution-v0.1":
...
def validate_candidates(row: dict[str, Any], row_index: int, verification_ids: set[str], errors: list[str]) -> None:
    ...
    for candidate_index, candidate in enumerate(candidates):
        ...
        use_boundary = str(candidate.get("useBoundary", ""))
        if "individual" not in use_boundary and "artifact" not in use_boundary:
...
def validate_row(
    ...
    source_context = " ".join(
        str(extracted.get(field, "")) for field in ["sourceAccessStatus", "sourceRole", "nextAction"]
    )
    if task_id in NO_ABSTRACT_TASK_IDS and "no-abstract" not in source_context:
        fail(errors, f"{task_id} extraction row must preserve no-abstract source issue status")
    if task_id == "C2LTB4-EXT-002" and "duplicate" not in source_context:
        fail(errors, f"{task_id} extraction row must preserve duplicate source issue status")
...
def validate_rows(
    ...
    if [row.get("taskId") for row in rows if isinstance(row, dict)] != EXPECTED_TASK_IDS:
        fail(errors, "resolutionRows must cover expected C2-LT-B4 issue task IDs in order")
...
def validate_top_level(data: dict[str, Any], errors: list[str]) -> None:
    ...
    required_next = require_string_list(data.get("requiredNextArtifacts"), "requiredNextArtifacts", errors, 6)
    ...
    if not any("manual-fulltext" in item or "source-resolution" in item for item in required_next):
        fail(errors, "requiredNextArtifacts must include source-resolution/manual fulltext next step")
...
def main() -> int:
    errors: list[str] = []
    data = load_json(REGISTER_PATH, errors, "C2-LT-B4 source-resolution register")
    ...
    if errors:
        ...
        return 1
    print("C2-LT-B4 source-resolution register audit ok: rows=4 candidates=8 model=blocked")
    return 0
