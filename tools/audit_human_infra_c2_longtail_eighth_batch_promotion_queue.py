...
QUEUE_PATH = ROOT / "docs/reference/human-infra-c2-longtail-eighth-batch-promotion-queue.json"
...
SCHEMA = "human-infra.c2-longtail-eighth-batch-promotion-queue.v1"
STATUS = "active-eighth-batch-queue-model-blocked"
BATCH_ID = "C2-LT-B8"
EXPECTED_ROWS = 12
EXPECTED_SOURCES = 24
REGISTER_LINK = "human-infra-c2-longtail-eighth-batch-promotion-queue.json"
SCRIPT_LINK = "audit_human_infra_c2_longtail_eighth_batch_promotion_queue.py"
...
REQUIRED_INDEX_LINKS = {
    ...
    "Makefile": "c2-longtail-eighth-batch-promotion-audit",
...
def validate_candidate_source(ref: Any, row_index: int, source_index: int, seen_refs: set[str], errors: list[str]) -> None:
    ...
    if source_id:
        ...
        if not source_id.startswith("C2LTB8-SRC-"):
            fail(errors, f"{context}.sourceRefId must start with C2LTB8-SRC-")
    ...
    if ref.get("selectionTrace") != "web-checked-source-candidate-2026-07-03":
        fail(errors, f"{context}.selectionTrace must record 2026-07-03 web check")
...
def main() -> int:
    errors: list[str] = []
    data = load_json(QUEUE_PATH, errors, "C2-LT-B8 promotion queue")
    ...
    if data:
        ...
        if not isinstance(source, dict) or "seventhBatchPromotionQueue" not in source:
            fail(errors, "sourceOfTruth must include previous B1-B7 queues")
        ...
        if not isinstance(scope, dict):
            ...
            for phrase in ["does not complete Source Cards", "does not complete independent fresh review", "does not admit any domain into calibrated prediction", "B1/B2/B3/B4/B5/B6/B7"]:
    ...
    if errors:
        ...
        return 1
    print(f"C2-LT-B8 promotion queue audit ok: domains={EXPECTED_ROWS} sources={EXPECTED_SOURCES}")
    return 0
