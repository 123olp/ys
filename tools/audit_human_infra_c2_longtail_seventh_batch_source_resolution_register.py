#!/usr/bin/env python3
"""审计 C2-LT-B7 source-resolution 账本。"""
...
REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-seventh-batch-source-resolution-register.json"
QUEUE_PATH = ROOT / "docs/reference/human-infra-c2-longtail-seventh-batch-source-extraction-queue.json"
EXTRACTION_PATH = ROOT / "docs/reference/human-infra-c2-longtail-seventh-batch-source-extraction-register.json"
LOCAL_REVIEW_PATH = ROOT / "docs/reference/human-infra-c2-longtail-seventh-batch-local-review-register.json"

SCHEMA = "human-infra.c2-longtail-seventh-batch-source-resolution-register.v1"
STATUS = "active-c2ltb7-source-resolution-register-independent-fresh-review-required"
REGISTER_LINK = "human-infra-c2-longtail-seventh-batch-source-resolution-register.json"
SCRIPT_LINK = "audit_human_infra_c2_longtail_seventh_batch_source_resolution_register.py"
...
DYNAMIC_REGISTRATION_TASK_IDS = {"C2LTB7-EXT-021"}
DUPLICATE_SOURCE_LINEAGE_TASK_IDS = {
    "C2LTB7-EXT-019",
    "C2LTB7-EXT-022",
}
EXPECTED_CANDIDATE_COUNT = 7
EXPECTED_VERIFICATION_IDS = {
    "C2LTB7-SR-VERIFY-CDC-TRANSPLANT-SAFETY-20260703",
    "C2LTB7-SR-VERIFY-FDA-HCTP-DONOR-ELIGIBILITY-20260703",
    "C2LTB7-SR-VERIFY-DONATELIFE-REGISTER-ROUTE-20260703",
    "C2LTB7-SR-VERIFY-FDA-XENOTRANSPLANTATION-20260703",
}
...
def validate_scope(data: dict[str, Any], rows: list[dict[str, Any]], errors: list[str]) -> None:
    ...
    expected = {
        ...
        "fdaRouteIssueCount": len(FDA_ROUTE_TASK_IDS),
        "dynamicRegistrationRouteIssueCount": len(DYNAMIC_REGISTRATION_TASK_IDS),
        "duplicateSourceLineageIssueCount": len(DUPLICATE_SOURCE_LINEAGE_TASK_IDS),
        "resolutionCandidateCount": EXPECTED_CANDIDATE_COUNT,
    }
    if scope.get("batchId") != "C2-LT-B7":
        fail(errors, "scope.batchId must be C2-LT-B7")
    if scope.get("resolutionLevel") != "c2ltb7-local-review-issue-row-source-resolution-v0.1":
...
def validate_resolution_rows(data: dict[str, Any], verification_ids: set[str], errors: list[str]) -> None:
    ...
    if [row.get("taskId") for row in rows if isinstance(row, dict)] != EXPECTED_TASK_IDS:
        fail(errors, "resolutionRows task order must match expected C2-LT-B7 issue rows")
...
def main() -> int:
    ...
    print(
        "C2-LT-B7 source-resolution register audit ok: "
