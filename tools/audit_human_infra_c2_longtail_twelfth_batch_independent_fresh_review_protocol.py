#!/usr/bin/env python3
"""审计 C2 长尾第十二批 independent fresh review 协议。"""
...
LOCAL_REVIEW_PATH = ROOT / "docs/reference/human-infra-c2-longtail-twelfth-batch-local-review-register.json"
EXTRACTION_PATH = ROOT / "docs/reference/human-infra-c2-longtail-twelfth-batch-source-extraction-register.json"

SCHEMA = "human-infra.c2-longtail-twelfth-batch-independent-fresh-review-protocol.v1"
STATUS = "active-review-protocol"
PROTOCOL_LINK = "human-infra-c2-longtail-twelfth-batch-independent-fresh-review-protocol.json"
AUDIT_SCRIPT = "audit_human_infra_c2_longtail_twelfth_batch_independent_fresh_review_protocol.py"
AUDIT_TARGET = "c2-longtail-twelfth-batch-independent-fresh-review-protocol-audit"
...
def local_review_batches(errors: list[str]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    local = load_json(LOCAL_REVIEW_PATH, errors, "C2-LT-B12 local review register")
    ...
    if not isinstance(scope, dict):
        ...
        if scope.get("sourceResolutionIssueCount") != 0:
            fail(errors, "C2-LT-B12 local review must preserve sourceResolutionIssueCount=0 for this protocol")
        if scope.get("localReviewedExtractionTaskCount") != len(EXPECTED_TASK_IDS):
            fail(errors, "C2-LT-B12 local review must cover all 24 rows")
    return local, [batch for batch in batches if isinstance(batch, dict)]


def extraction_domain_count(errors: list[str]) -> int:
    extraction = load_json(EXTRACTION_PATH, errors, "C2-LT-B12 source extraction register")
    ...
    if task_ids != EXPECTED_TASK_IDS:
        fail(errors, "source extraction register rows must cover C2LTB12-EXT-001 through C2LTB12-EXT-024")
...
def validate_top_level(protocol: dict[str, Any], errors: list[str]) -> None:
    ...
    if set(require_string_list(protocol.get("blockedUses"), "blockedUses", errors, len(REQUIRED_BLOCKED_USES))) != REQUIRED_BLOCKED_USES:
        fail(errors, "blockedUses must match required B12 blocked uses")
...
def validate_scope(protocol: dict[str, Any], domain_count: int, errors: list[str]) -> None:
    ...
    for phrase in [
        ...
        if phrase not in non_claims:
            fail(errors, f"scope.nonClaims must preserve B12 advice boundary: {phrase}")
...
def validate_batches(protocol: dict[str, Any], local_batches: list[dict[str, Any]], errors: list[str]) -> None:
    ...
    if covered != EXPECTED_TASK_IDS:
        fail(errors, "reviewBatches must cover C2LTB12-EXT-001 through C2LTB12-EXT-024 exactly once in order")
    ...
    for relative_path in REQUIRED_INDEX_FILES:
        ...
        if relative_path == "Makefile":
            if AUDIT_TARGET not in text:
                fail(errors, "Makefile must expose twelfth-batch independent fresh review protocol audit target")
            if AUDIT_SCRIPT not in text:
                fail(errors, "Makefile must run twelfth-batch independent fresh review protocol audit script")
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    print("C2 longtail twelfth-batch independent fresh review protocol audit ok: tasks=24 batches=2 verdicts=0")
    return 0
