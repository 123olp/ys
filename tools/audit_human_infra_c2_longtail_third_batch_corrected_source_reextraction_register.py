#!/usr/bin/env python3
"""审计 C2-LT-B3 corrected source re-extraction 完成寄存器。"""
...
REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-third-batch-corrected-source-reextraction-register.json"
QUEUE_PATH = ROOT / "docs/reference/human-infra-c2-longtail-third-batch-corrected-source-reextraction-queue.json"

SCHEMA = "human-infra.c2ltb3-corrected-source-reextraction-register.v1"
STATUS = "active-c2ltb3-corrected-source-reextracted-model-blocked"
REGISTER_LINK = "human-infra-c2-longtail-third-batch-corrected-source-reextraction-register.json"
SCRIPT_LINK = "audit_human_infra_c2_longtail_third_batch_corrected_source_reextraction_register.py"
BATCH_ID = "C2-LT-B3"
...
def validate_rows(register: dict[str, Any], queued: dict[str, dict[str, Any]], errors: list[str]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(rows):
        ...
        if any(marker in source_access for marker in ("403", "route-only", "duplicate-split-route")):
            if readiness not in LIMITED_PROMOTION_STATES:
                fail(errors, f"{task_id}.artifactPromotionReadiness must preserve route/split limitation")
...
def main() -> int:
    errors: list[str] = []
    register = load_json(REGISTER_PATH, errors, "C2-LT-B3 corrected source re-extraction register")
    ...
    if register:
        ...
        if not any(row.get("artifactPromotionReadiness") == "eligible-for-independent-fresh-review-before-bounded-artifact-promotion" for row in rows):
            fail(errors, "at least one row should be eligible for independent fresh review candidate status")
        if not any(row.get("artifactPromotionReadiness") in LIMITED_PROMOTION_STATES for row in rows):
            fail(errors, "at least one row should preserve route/split blocking")

    if errors:
        print("C2-LT-B3 corrected source re-extraction register audit failed:")
        for error in errors:
            print(f"- {error}")
        return 1
    print("C2-LT-B3 corrected source re-extraction register audit passed.")
    return 0
