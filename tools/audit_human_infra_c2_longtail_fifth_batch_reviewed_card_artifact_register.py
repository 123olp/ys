#!/usr/bin/env python3
"""审计 C2-LT-B5 reviewed card artifact 账本。"""
...
REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-fifth-batch-reviewed-card-artifact-register.json"
VERDICT_PATH = (
    ROOT
...
SCHEMA = "human-infra.c2ltb5-reviewed-card-artifact-register.v1"
STATUS = "active-c2ltb5-reviewed-card-artifact-register-model-blocked"
REGISTER_LINK = "human-infra-c2-longtail-fifth-batch-reviewed-card-artifact-register.json"
SCRIPT_LINK = "audit_human_infra_c2_longtail_fifth_batch_reviewed_card_artifact_register.py"
BATCH_ID = "C2-LT-B5"
ELIGIBLE_DECISION = "eligible-for-bounded-reviewed-artifact-prep"
ARTIFACT_DECISION = "filled-from-eligible-c2ltb5-fresh-review"
ARTIFACT_STATUS = "filled-reviewed-c2ltb5-artifact-model-blocked"
MODEL_DECISION = "blocked-pending-c2ltb5-reviewed-artifact-gates-and-calibrated-model-validation"
...
def validate_scope(
    ...
    if scope.get("artifactLevel") != "c2ltb5-independent-fresh-review-verdict-to-reviewed-card-artifact":
...
def main() -> int:
    errors: list[str] = []
    register = load_json(REGISTER_PATH, errors, "B5 reviewed artifact register")
    verdict = load_json(VERDICT_PATH, errors, "B5 independent fresh-review verdict register")
    ...
    if errors:
        print("C2-LT-B5 reviewed artifact audit failed:")
        for error in errors:
            print(f"- {error}")
        return 1
    print("C2-LT-B5 reviewed artifact audit ok: artifacts=102 blocked=13 model=blocked")
    return 0
