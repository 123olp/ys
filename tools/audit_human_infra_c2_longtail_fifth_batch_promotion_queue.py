...
QUEUE_PATH = ROOT / "docs/reference/human-infra-c2-longtail-fifth-batch-promotion-queue.json"
...
SECOND_BATCH_PATH = ROOT / "docs/reference/human-infra-c2-longtail-second-batch-promotion-queue.json"
THIRD_BATCH_PATH = ROOT / "docs/reference/human-infra-c2-longtail-third-batch-promotion-queue.json"
FOURTH_BATCH_PATH = ROOT / "docs/reference/human-infra-c2-longtail-fourth-batch-promotion-queue.json"

SCHEMA = "human-infra.c2-longtail-fifth-batch-promotion-queue.v1"
STATUS = "active-fifth-batch-queue-model-blocked"
EXPECTED_BATCH_ID = "C2-LT-B5"
EXPECTED_COVERAGE_STATUS = "c2-longtail-uncovered"
REGISTER_LINK = "human-infra-c2-longtail-fifth-batch-promotion-queue.json"
SCRIPT_LINK = "audit_human_infra_c2_longtail_fifth_batch_promotion_queue.py"
...
def validate_scope(data: dict[str, Any], row_count: int, errors: list[str]) -> None:
    ...
    for phrase in [
        ...
        "does not mark C2-LT-B1 corrected blocked rows",
        "C2-LT-B2 downgrade-before-fill row",
        "C2-LT-B3 issue",
        "C2-LT-B4 duplicate",
    ]:
...
def validate_candidate_source(ref: Any, path: str, seen_refs: set[str], errors: list[str]) -> None:
    ...
    if source_ref_id:
        ...
        if not source_ref_id.startswith("C2LTB5-SRC-"):
            fail(errors, f"{path}.sourceRefId must start with C2LTB5-SRC-")
...
def validate_row(
    ...
    previous_batch_domains: set[str],
    ...
    if domain_id in previous_batch_domains:
        fail(errors, f"{domain_id} must not duplicate C2-LT-B1, C2-LT-B2, C2-LT-B3 or C2-LT-B4")
...
def validate_summary(data: dict[str, Any], rows: list[Any], errors: list[str]) -> None:
    ...
    if summary.get("batchId") != EXPECTED_BATCH_ID:
        fail(errors, "summary.batchId must be C2-LT-B5")
...
def main() -> int:
    errors: list[str] = []
    queue = load_json(QUEUE_PATH, errors, "C2 longtail fifth-batch queue")
    ...
    second_batch = load_json(SECOND_BATCH_PATH, errors, "C2 longtail second-batch queue")
    third_batch = load_json(THIRD_BATCH_PATH, errors, "C2 longtail third-batch queue")
    fourth_batch = load_json(FOURTH_BATCH_PATH, errors, "C2 longtail fourth-batch queue")
    ...
    second_rows = second_batch.get("batchRows") if second_batch else []
    if not isinstance(second_rows, list):
        fail(errors, "second batch queue batchRows must be a list")
        second_rows = []
    second_batch_domains = {row.get("domainId") for row in second_rows if isinstance(row, dict)}
    third_rows = third_batch.get("batchRows") if third_batch else []
    if not isinstance(third_rows, list):
        fail(errors, "third batch queue batchRows must be a list")
        third_rows = []
    third_batch_domains = {row.get("domainId") for row in third_rows if isinstance(row, dict)}
    fourth_rows = fourth_batch.get("batchRows") if fourth_batch else []
    if not isinstance(fourth_rows, list):
        fail(errors, "fourth batch queue batchRows must be a list")
        fourth_rows = []
    fourth_batch_domains = {row.get("domainId") for row in fourth_rows if isinstance(row, dict)}
    previous_batch_domains = first_batch_domains | second_batch_domains | third_batch_domains | fourth_batch_domains
    ...
    for index, row in enumerate(rows):
        validate_row(row, index, coverage_by_domain, previous_batch_domains, seen_domains, seen_refs, errors)
    validate_indexes(errors)
    validate_summary(queue, rows, errors)

    if errors:
        print("C2 longtail fifth-batch promotion queue audit failed:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    print(
        "C2 longtail fifth-batch promotion queue audit ok: "
        f"batch=C2-LT-B5 domains={len(seen_domains)} candidate_sources={len(seen_refs)}"
    )
    return 0
