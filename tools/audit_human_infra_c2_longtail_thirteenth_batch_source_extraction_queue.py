#!/usr/bin/env python3
"""审计 C2 长尾第十三批 source-specific 深读任务队列。"""
...
PROMOTION_QUEUE_PATH = ROOT / "docs/reference/human-infra-c2-longtail-thirteenth-batch-promotion-queue.json"

SCHEMA = "human-infra.c2-longtail-thirteenth-batch-source-extraction-queue.v1"
STATUS = "active-c2ltb13-source-specific-extraction-queue-model-blocked"
BATCH_ID = "C2-LT-B13"
EXPECTED_DOMAINS = 12
EXPECTED_TASKS = 24
TASK_ID_RE = re.compile(r"^C2LTB13-EXT-(\d{3})$")
...
REQUIRED_INDEX_LINKS = {
    "docs/AGENTS.md": "human-infra-c2-longtail-thirteenth-batch-source-extraction-queue.json",
    "docs/reference/README.md": "human-infra-c2-longtail-thirteenth-batch-source-extraction-queue.json",
    "docs/reference/human-infra-maturity-roadmap.md": "human-infra-c2-longtail-thirteenth-batch-source-extraction-queue.json",
    "docs/reference/human-infra-maturity-gap-register.json": "human-infra-c2-longtail-thirteenth-batch-source-extraction-queue.json",
    "Makefile": "c2-longtail-thirteenth-batch-source-extraction-audit",
    "tools/README.md": "audit_human_infra_c2_longtail_thirteenth_batch_source_extraction_queue.py",
    "tools/AGENTS.md": "audit_human_infra_c2_longtail_thirteenth_batch_source_extraction_queue.py",
}
...
def validate_task(
    ...
    if not match:
        fail(errors, f"extractionTasks[{index}].taskId must match C2LTB13-EXT-###")
...
def main() -> int:
    errors: list[str] = []
    data = load_json(QUEUE_PATH, errors, "C2-LT-B13 source extraction queue")
    promotion = load_json(PROMOTION_QUEUE_PATH, errors, "C2-LT-B13 promotion queue")
    ...
    if data:
        ...
        for phrase in ["does not mean the sources have been read in context", "does not create reviewed artifacts", "does not resolve any prior B1-B12 blocked row"]:
    ...
    if errors:
        ...
        return 1
    print(f"C2-LT-B13 source extraction queue audit ok: domains={EXPECTED_DOMAINS} tasks={EXPECTED_TASKS}")
    return 0
