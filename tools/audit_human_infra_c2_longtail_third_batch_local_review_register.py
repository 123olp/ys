...
REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-third-batch-local-review-register.json"
QUEUE_PATH = ROOT / "docs/reference/human-infra-c2-longtail-third-batch-source-extraction-queue.json"
EXTRACTION_REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-third-batch-source-extraction-register.json"

SCHEMA = "human-infra.c2-longtail-third-batch-local-review-register.v1"
STATUS = "complete-local-source-context-review-independent-fresh-review-required"
REGISTER_LINK = "human-infra-c2-longtail-third-batch-local-review-register.json"
...
REQUIRED_NEXT_ARTIFACTS = {
    "c2ltb3-independent-fresh-review-protocol",
    "c2ltb3-independent-fresh-review-verdict-register",
...
def load_queue(errors: list[str]) -> dict[str, dict[str, Any]]:
    queue = load_json(QUEUE_PATH, errors, "C2-LT-B3 source extraction queue")
    ...
    if list(result) != EXPECTED_TASK_IDS:
        fail(errors, "queue task IDs must be C2LTB3-EXT-001 through C2LTB3-EXT-024")
    return result


def load_extraction_rows(errors: list[str]) -> dict[str, dict[str, Any]]:
    register = load_json(EXTRACTION_REGISTER_PATH, errors, "C2-LT-B3 source extraction register")
    ...
    for index, row in enumerate(rows):
        ...
        if task_id:
            result[task_id] = row
    if list(result) != EXPECTED_TASK_IDS:
        fail(errors, "source extraction register rows must be ordered as all 24 C2LTB3 task IDs")
    return result
...
def validate_scope(scope: Any, task_map: dict[str, dict[str, Any]], errors: list[str]) -> None:
    if not isinstance(scope, dict):
        ...
        return
    if require_string(scope.get("batchId"), "scope.batchId", errors) != "C2-LT-B3":
    ...
    if selected != EXPECTED_TASK_IDS:
        fail(errors, "scope.reviewedExtractionTaskIds must equal all 24 C2LTB3 task IDs")
...
def validate_extraction_row(row: dict[str, Any], task: dict[str, Any], errors: list[str]) -> None:
    ...
    if row.get("extractionStatus") != "completed-c2ltb3-source-context-extraction":
        fail(errors, f"{task_id}.extractionStatus must be completed-c2ltb3-source-context-extraction")
    for field in [
        "sourceRole",
        "exactClaimUse",
        "endpointDefinition",
    ...
    for index, finding in enumerate(findings):
        ...
        if task_id:
            ...
            if row is None:
                ...
                access_status = str(row.get("sourceAccessStatus", ""))
                if not any(marker in access_status for marker in ["mismatch", "forbidden"]):
                    fail(errors, f"{task_id} must have mismatch or forbidden sourceAccessStatus")
        ...
        if not any(marker in disposition for marker in ["source-resolution", "manual-access", "bounded-fresh-review"]):
            fail(errors, f"{context}.reviewDisposition must preserve source-resolution/manual-access boundary")
    if task_ids != EXPECTED_SOURCE_RESOLUTION_IDS:
        fail(errors, "sourceResolutionFindings must list the five expected C2-LT-B3 issue rows in order")
...
def validate_register_fields(data: dict[str, Any], errors: list[str]) -> None:
    ...
    if "c2ltb3-source-resolution-register-for-issue-rows" not in next_artifacts:
        fail(errors, "requiredNextArtifacts must include source-resolution register for C2-LT-B3 issue rows")
...
def main() -> int:
    errors: list[str] = []
    data = load_json(REGISTER_PATH, errors, "C2-LT-B3 local review register")
    ...
    if errors:
        ...
        return 1
    print(
        "C2 longtail third-batch local review register audit ok: "
        "batch=C2-LT-B3 reviewed=24 domains=12 source_resolution_issues=5"
    )
    return 0
