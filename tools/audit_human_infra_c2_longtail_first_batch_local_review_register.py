...
REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-first-batch-local-review-register.json"
QUEUE_PATH = ROOT / "docs/reference/human-infra-c2-longtail-first-batch-source-extraction-queue.json"
EXTRACTION_REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-first-batch-source-extraction-register.json"

SCHEMA = "human-infra.c2-longtail-first-batch-local-review-register.v1"
STATUS = "complete-local-source-context-review-independent-fresh-review-required"
REGISTER_LINK = "human-infra-c2-longtail-first-batch-local-review-register.json"
...
REQUIRED_NEXT_ARTIFACTS = {
    "c2ltb1-independent-fresh-review-protocol",
    "c2ltb1-independent-fresh-review-verdict-register",
...
def load_queue(errors: list[str]) -> dict[str, dict[str, Any]]:
    queue = load_json(QUEUE_PATH, errors, "C2-LT-B1 source extraction queue")
    ...
    if list(result) != EXPECTED_TASK_IDS:
        fail(errors, "queue task IDs must be C2LTB1-EXT-001 through C2LTB1-EXT-048")
    return result


def load_extraction_rows(errors: list[str]) -> dict[str, dict[str, Any]]:
    register = load_json(EXTRACTION_REGISTER_PATH, errors, "C2-LT-B1 source extraction register")
    ...
    for index, row in enumerate(rows):
        ...
        if task_id:
            result[task_id] = row
    if list(result) != EXPECTED_TASK_IDS:
        fail(errors, "source extraction register rows must be ordered as all 48 C2LTB1 task IDs")
    return result
...
def validate_scope(scope: Any, task_map: dict[str, dict[str, Any]], errors: list[str]) -> None:
    ...
    if selected != EXPECTED_TASK_IDS:
        fail(errors, "scope.reviewedExtractionTaskIds must equal all 48 C2LTB1 task IDs")
...
def validate_batches(
    ...
    if not isinstance(batches, list) or len(batches) != 4:
        fail(errors, "reviewBatches must contain exactly four batch objects")
        return
    covered: list[str] = []
    ...
    if covered != EXPECTED_TASK_IDS:
        fail(errors, "reviewBatches must cover all 48 task IDs exactly once in order")
...
def main() -> int:
    errors: list[str] = []
    data = load_json(REGISTER_PATH, errors, "C2-LT-B1 local review register")
    ...
    if errors:
        ...
        return 1
    print("C2 longtail first-batch local review register audit ok: batch=C2-LT-B1 reviewed=48 domains=24")
    return 0
