#!/usr/bin/env python3
"""审计 C2-LT-B5 independent fresh-review verdict 账本。"""
...
REGISTER_PATH = (
    ROOT
    / "docs/reference/human-infra-c2-longtail-fifth-batch-independent-fresh-review-verdict-register.json"
)
SOURCE_EXTRACTION_PATH = (
    ROOT / "docs/reference/human-infra-c2-longtail-fifth-batch-source-extraction-register.json"
)
...
MANUAL_EXTRACTION_PATH = (
    ROOT / "docs/reference/human-infra-c2-longtail-fifth-batch-manual-fulltext-extraction-register.json"
)

SCHEMA = "human-infra.c2ltb5-independent-fresh-review-verdict-register.v1"
STATUS = "active-c2ltb5-independent-fresh-review-verdict-register-model-blocked"
REGISTER_LINK = "human-infra-c2-longtail-fifth-batch-independent-fresh-review-verdict-register.json"
SCRIPT_LINK = "audit_human_infra_c2_longtail_fifth_batch_independent_fresh_review_verdict_register.py"
BATCH_ID = "C2-LT-B5"
...
EXPECTED_MANUAL_ELIGIBLE_ROWS = {"C2LTB5-MFEXT-007"}
...
def source_extraction_rows(errors: list[str]) -> dict[str, dict[str, Any]]:
    data = load_json(SOURCE_EXTRACTION_PATH, errors, "C2-LT-B5 source extraction register")
...
def issue_task_ids(errors: list[str]) -> set[str]:
    data = load_json(LOCAL_REVIEW_PATH, errors, "C2-LT-B5 local review register")
...
def manual_rows(errors: list[str]) -> dict[str, dict[str, Any]]:
    data = load_json(MANUAL_EXTRACTION_PATH, errors, "C2-LT-B5 manual/fulltext extraction register")
...
def validate_source_reviews(
    ...
    for index, review in enumerate(reviews):
        ...
        if review_id and review_id != f"C2LTB5-FRV-{index + 1:03d}":
            fail(errors, f"{review_id} must follow sequential C2LTB5-FRV-### order")
...
def validate_manual_reviews(
    ...
    for index, review in enumerate(reviews):
        ...
        if review_id and review_id != f"C2LTB5-MFFRV-{index + 1:03d}":
            fail(errors, f"{review_id} must follow sequential C2LTB5-MFFRV-### order")
...
def validate_scope_and_summary(
    ...
    if scope.get("reviewLevel") != "c2ltb5-independent-fresh-review-v0.1":
    ...
    if set(manual_eligible) != EXPECTED_MANUAL_ELIGIBLE_ROWS:
        fail(errors, "summary eligible manual rows must match expected retained PMC route")
    ...
    for phrase in ("Sixteen", "Intercompartmental communication", "Twelve route-only", "calibrated prediction"):
...
def validate_index_links(register: dict[str, Any], errors: list[str]) -> None:
    ...
    for relative_path in REQUIRED_INDEX_FILES:
        ...
        if REGISTER_LINK not in text:
            fail(errors, f"index file does not reference B5 fresh-review verdict register: {relative_path}")
...
def main() -> int:
    errors: list[str] = []
    register = load_json(REGISTER_PATH, errors, "C2-LT-B5 independent fresh-review verdict register")
    ...
    if errors:
        print("C2-LT-B5 independent fresh-review verdict audit failed:")
        for error in errors:
            print(f"- {error}")
        return 1
    print("C2-LT-B5 independent fresh-review verdict audit ok: source=16 manual=14 eligible=17 blocked=13 model=blocked")
    return 0
