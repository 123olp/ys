...
REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-seventh-batch-local-review-register.json"
QUEUE_PATH = ROOT / "docs/reference/human-infra-c2-longtail-seventh-batch-source-extraction-queue.json"
EXTRACTION_REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-seventh-batch-source-extraction-register.json"

SCHEMA = "human-infra.c2-longtail-seventh-batch-local-review-register.v1"
STATUS = "complete-local-source-context-review-independent-fresh-review-required"
REGISTER_LINK = "human-infra-c2-longtail-seventh-batch-local-review-register.json"
EXPECTED_TASK_IDS = [f"C2LTB7-EXT-{index:03d}" for index in range(1, 25)]
...
REQUIRED_NEXT_ARTIFACTS = {
    "c2ltb7-independent-fresh-review-protocol",
    "c2ltb7-independent-fresh-review-verdict-register",
    "c2ltb7-source-resolution-register-for-issue-rows",
...
def load_queue(errors: list[str]) -> dict[str, dict[str, Any]]:
    queue = load_json(QUEUE_PATH, errors, "C2-LT-B7 source extraction queue")
    ...
    if list(result) != EXPECTED_TASK_IDS:
        fail(errors, "queue task IDs must be C2LTB7-EXT-001 through C2LTB7-EXT-024")
    return result


def load_extraction_rows(errors: list[str]) -> dict[str, dict[str, Any]]:
    register = load_json(EXTRACTION_REGISTER_PATH, errors, "C2-LT-B7 source extraction register")
    ...
    for index, row in enumerate(rows):
        ...
        if task_id:
            result[task_id] = row
    if list(result) != EXPECTED_TASK_IDS:
        fail(errors, "source extraction register rows must be ordered as all 24 C2LTB7 task IDs")
    return result
...
def validate_scope(scope: Any, task_map: dict[str, dict[str, Any]], errors: list[str]) -> None:
    if not isinstance(scope, dict):
        ...
        return
    if require_string(scope.get("batchId"), "scope.batchId", errors) != "C2-LT-B7":
        fail(errors, "scope.batchId must be C2-LT-B7")
    ...
    if selected != EXPECTED_TASK_IDS:
        fail(errors, "scope.reviewedExtractionTaskIds must equal all 24 C2LTB7 task IDs")
...
def validate_extraction_row(row: dict[str, Any], task: dict[str, Any], errors: list[str]) -> None:
    ...
    if row.get("extractionStatus") != "completed-c2ltb7-source-context-extraction":
        fail(errors, f"{task_id}.extractionStatus must be completed-c2ltb7-source-context-extraction")
    for field in [
        "sourceRole",
        "exactClaimUse",
        "endpointDefinition",
...
def validate_source_resolution_findings(findings: Any, extraction_rows: dict[str, dict[str, Any]], errors: list[str]) -> None:
    ...
    for index, finding in enumerate(findings):
        ...
        if task_id:
            ...
            if row is None:
                ...
                markers = ["404", "duplicate", "dynamic", "202", "route", "lineage", "source-resolution"]
                if not any(marker in source_context for marker in markers):
                    fail(errors, f"{task_id} must preserve route/dynamic/duplicate/source-resolution context")
        ...
        if not any(marker in disposition for marker in ["source-resolution", "manual-route", "bounded-fresh-review", "evidence-counting"]):
    ...
    if task_ids != EXPECTED_SOURCE_RESOLUTION_IDS:
        fail(errors, "sourceResolutionFindings must list the six expected C2-LT-B7 issue rows in order")
...
def validate_register_fields(data: dict[str, Any], errors: list[str]) -> None:
    ...
    if "c2ltb7-source-resolution-register-for-issue-rows" not in next_artifacts:
        fail(errors, "requiredNextArtifacts must include source-resolution register for C2-LT-B7 issue rows")
...
def main() -> int:
    errors: list[str] = []
    data = load_json(REGISTER_PATH, errors, "C2-LT-B7 local review register")
    ...
    if errors:
        ...
        return 1
    print(
        "C2 longtail seventh-batch local review register audit ok: "
        "batch=C2-LT-B7 reviewed=24 domains=12 source_resolution_issues=6"
    )
    return 0
