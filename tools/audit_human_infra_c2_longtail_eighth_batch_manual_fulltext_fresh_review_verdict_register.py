#!/usr/bin/env python3
"""审计 C2-LT-B8 manual/fulltext fresh-review 判定账本。"""
...
REGISTER_PATH = (
    ROOT
    / "docs/reference/human-infra-c2-longtail-eighth-batch-manual-fulltext-fresh-review-verdict-register.json"
)
MANUAL_EXTRACTION_PATH = (
    ROOT / "docs/reference/human-infra-c2-longtail-eighth-batch-manual-fulltext-extraction-register.json"
)

SCHEMA = "human-infra.c2ltb8-manual-fulltext-fresh-review-verdict-register.v1"
STATUS = "active-c2ltb8-manual-fulltext-fresh-review-verdict-register-model-blocked"
REGISTER_LINK = "human-infra-c2-longtail-eighth-batch-manual-fulltext-fresh-review-verdict-register.json"
SCRIPT_LINK = "audit_human_infra_c2_longtail_eighth_batch_manual_fulltext_fresh_review_verdict_register.py"
BATCH_ID = "C2-LT-B8"
...
BLOCKED_PROMOTION_DECISIONS = {
    ...
    "blocked-route-only-no-direct-artifact-fill",
}
...
def validate_reviews(
    ...
    for index, review in enumerate(reviews):
        ...
        if review_id and review_id != f"C2LTB8-MFFRV-{index + 1:03d}":
            fail(errors, f"{review_id} must follow sequential C2LTB8-MFFRV-### order")
...
def validate_scope(register: dict[str, Any], reviews: list[dict[str, Any]], errors: list[str]) -> None:
    ...
    if scope.get("reviewLevel") != "c2ltb8-manual-fulltext-fresh-review-v0.1":
    ...
    for key, expected_value in {
        "pmcReadableCandidateRowCount": 5,
        "ncbiOaPackageCandidateRowCount": 2,
        "pmcReadableNotNcbioaPackageRowCount": 3,
        "livingNeuralComputationOverclaimGuardedRowCount": 1,
    }.items():
        actual = require_int(scope.get(key), f"scope.{key}", errors)
        if actual is not None and actual != expected_value:
            fail(errors, f"scope.{key} must equal {expected_value}")
    non_claims = "\n".join(require_string_list(scope.get("nonClaims"), "scope.nonClaims", errors, min_len=4))
    for phrase in ("does not create reviewed artifacts", "does not open model admission", "does not authorize", "PMC readability"):
...
def validate_summary(register: dict[str, Any], reviews: list[dict[str, Any]], errors: list[str]) -> None:
    ...
    for phrase in ("ADHD", "long-COVID", "vestibular", "dysphagia", "Living neural computation"):
...
def main() -> int:
    ...
    if errors:
        print("C2-LT-B8 manual/fulltext fresh-review verdict audit failed:")
        for error in errors:
            print(f"- {error}")
        return 1
    print("C2-LT-B8 manual/fulltext fresh-review verdict audit ok: reviewed=19 eligible=5 blocked=14 model=blocked")
    return 0
