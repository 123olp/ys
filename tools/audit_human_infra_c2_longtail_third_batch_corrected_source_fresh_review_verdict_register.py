#!/usr/bin/env python3
"""审计 C2-LT-B3 corrected source fresh-review 判定账本。"""
...
REGISTER_PATH = (
    ROOT
    / "docs/reference/human-infra-c2-longtail-third-batch-corrected-source-fresh-review-verdict-register.json"
)
EXTRACTION_REGISTER_PATH = (
    ROOT
    / "docs/reference/human-infra-c2-longtail-third-batch-corrected-source-reextraction-register.json"
)

SCHEMA = "human-infra.c2ltb3-corrected-source-fresh-review-verdict-register.v1"
STATUS = "active-c2ltb3-corrected-source-fresh-review-verdict-register-model-blocked"
REGISTER_LINK = "human-infra-c2-longtail-third-batch-corrected-source-fresh-review-verdict-register.json"
SCRIPT_LINK = "audit_human_infra_c2_longtail_third_batch_corrected_source_fresh_review_verdict_register.py"
BATCH_ID = "C2-LT-B3"
...
BLOCKED_PROMOTION_DECISIONS = {
    "blocked-duplicate-split-route-no-direct-artifact-fill",
    "blocked-route-or-manual-access-required-no-direct-artifact-fill",
}
...
EXPECTED_BLOCKED_ROWS = {"C2LTB3-CREXT-004"}
...
def validate_reviews(register: dict[str, Any], rows_by_task: dict[str, dict[str, Any]], errors: list[str]) -> list[dict[str, Any]]:
    ...
    for index, review in enumerate(reviews):
        ...
        if review_id and review_id != f"C2LTB3-CFRV-{index + 1:03d}":
            fail(errors, f"{review_id} must follow sequential C2LTB3-CFRV-### order")
        ...
        if task_id in EXPECTED_ELIGIBLE_ROWS:
        ...
        elif task_id in EXPECTED_BLOCKED_ROWS:
            if decision != "blocked-duplicate-split-route-no-direct-artifact-fill":
                fail(errors, f"{review_id}.artifactPromotionDecision must be duplicate split blocked")
            ...
            fail(errors, f"unexpected corrected source row: {task_id}")

        if task_id == "C2LTB3-CREXT-006":
            access = require_string(review.get("sourceAccessVerdict"), f"{review_id}.sourceAccessVerdict", errors)
            finding = require_string(review.get("freshReviewFinding"), f"{review_id}.freshReviewFinding", errors)
            if "HTTP 200" not in finding or "supersedes-prior-403" not in access:
                fail(errors, f"{review_id} must record the AAO-HNS access-status update")
        if task_id == "C2LTB3-CREXT-007":
            context = require_string(review.get("sourceContextVerdict"), f"{review_id}.sourceContextVerdict", errors)
            finding = require_string(review.get("freshReviewFinding"), f"{review_id}.freshReviewFinding", errors)
            if "not-independent-second-source" not in context or "must not be counted as a second independent source" not in finding:
                fail(errors, f"{review_id} must preserve companion-source duplicate boundary")
...
def validate_scope(register: dict[str, Any], reviews: list[dict[str, Any]], errors: list[str]) -> None:
    ...
    if scope.get("reviewLevel") != "c2ltb3-corrected-source-fresh-review-v0.1":
...
def validate_summary(register: dict[str, Any], reviews: list[dict[str, Any]], errors: list[str]) -> None:
    ...
    if set(eligible) != EXPECTED_ELIGIBLE_ROWS:
        fail(errors, "summary eligible rows must match expected corrected fresh-review decisions")
    if set(blocked) != EXPECTED_BLOCKED_ROWS:
        fail(errors, "summary blocked rows must match expected corrected fresh-review decisions")
    ...
    new_facts = "\n".join(require_string_list(summary.get("newFreshReviewFacts"), "summary.newFreshReviewFacts", errors, min_len=2))
    if "AAO-HNS publisher route" not in new_facts or "duplicate/split" not in new_facts:
        fail(errors, "summary.newFreshReviewFacts must include access update and duplicate/split boundary")
...
def main() -> int:
    ...
    if errors:
        print("C2-LT-B3 corrected source fresh-review verdict audit failed:")
        for error in errors:
            print(f"- {error}")
        return 1
    print("C2-LT-B3 corrected source fresh-review verdict audit passed.")
    return 0
