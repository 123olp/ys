...
QUEUE_PATH = ROOT / "docs/reference/human-infra-c2-longtail-tenth-batch-promotion-queue.json"
...
SCHEMA = "human-infra.c2-longtail-tenth-batch-promotion-queue.v1"
STATUS = "active-tenth-batch-queue-model-blocked"
BATCH_ID = "C2-LT-B10"
EXPECTED_ROWS = 12
EXPECTED_SOURCES = 24
REGISTER_LINK = "human-infra-c2-longtail-tenth-batch-promotion-queue.json"
SCRIPT_LINK = "audit_human_infra_c2_longtail_tenth_batch_promotion_queue.py"
...
REQUIRED_INDEX_LINKS = {
    ...
    "Makefile": "c2-longtail-tenth-batch-promotion-audit",
...
def validate_candidate_source(ref: Any, row_index: int, source_index: int, seen_refs: set[str], errors: list[str]) -> None:
    ...
    if source_id:
        ...
        if not source_id.startswith("C2LTB10-SRC-"):
            fail(errors, f"{context}.sourceRefId must start with C2LTB10-SRC-")
...
def main() -> int:
    errors: list[str] = []
    data = load_json(QUEUE_PATH, errors, "C2-LT-B10 promotion queue")
    ...
    if data:
        ...
        if not isinstance(source, dict) or "ninthBatchPromotionQueue" not in source:
            fail(errors, "sourceOfTruth must include previous B1-B9 queues")
    ...
    if errors:
        ...
        return 1
    print(f"C2-LT-B10 promotion queue audit ok: domains={EXPECTED_ROWS} sources={EXPECTED_SOURCES}")
    return 0
