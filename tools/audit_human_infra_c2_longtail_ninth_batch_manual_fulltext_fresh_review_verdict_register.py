#!/usr/bin/env python3
"""审计 C2-LT-B9 manual/fulltext fresh-review 判定账本。"""
...
REGISTER_PATH = (
    ROOT
    / "docs/reference/human-infra-c2-longtail-ninth-batch-manual-fulltext-fresh-review-verdict-register.json"
)
MANUAL_EXTRACTION_PATH = (
    ROOT / "docs/reference/human-infra-c2-longtail-ninth-batch-manual-fulltext-extraction-register.json"
)

SCHEMA = "human-infra.c2ltb9-manual-fulltext-fresh-review-verdict-register.v1"
STATUS = "active-c2ltb9-manual-fulltext-fresh-review-verdict-register-model-blocked"
REGISTER_LINK = "human-infra-c2-longtail-ninth-batch-manual-fulltext-fresh-review-verdict-register.json"
SCRIPT_LINK = "audit_human_infra_c2_longtail_ninth_batch_manual_fulltext_fresh_review_verdict_register.py"
BATCH_ID = "C2-LT-B9"
FRESH_REVIEW_DATE = "2026-07-04"
...
BLOCKED_PROMOTION_DECISIONS = {
    "blocked-route-metadata-no-direct-artifact-fill",
    "blocked-publisher-route-no-direct-artifact-fill",
    "blocked-companion-route-no-direct-artifact-fill",
    "blocked-redirect-provenance-no-direct-artifact-fill",
}
...
def validate_reviews(
    ...
    for index, review in enumerate(reviews):
        ...
        if review_id and review_id != f"C2LTB9-MFFRV-{index + 1:03d}":
            fail(errors, f"{review_id} must follow sequential C2LTB9-MFFRV-### order")
        ...
        if review.get("freshReviewDate") != FRESH_REVIEW_DATE:
            fail(errors, f"{review_id}.freshReviewDate must be {FRESH_REVIEW_DATE}")
...
def validate_scope(register: dict[str, Any], reviews: list[dict[str, Any]], errors: list[str]) -> None:
    ...
    if scope.get("reviewLevel") != "c2ltb9-manual-fulltext-fresh-review-v0.1":
    ...
    expected = {
        ...
        "officialGuidelinePageCandidateRowCount": 1,
        "officialPolicyPageCandidateRowCount": 2,
        "officialPdfCandidateRowCount": 1,
    ...
    for phrase in (
        ...
        "does not treat PubMed",
        "does not authorize wound care",
    ):
...
def validate_summary(register: dict[str, Any], reviews: list[dict[str, Any]], errors: list[str]) -> None:
    ...
    for phrase in ("IDSA", "PubMed", "Medicaid", "EPSDT", "blocked"):
...
def main() -> int:
    ...
    if errors:
        print("C2-LT-B9 manual/fulltext fresh-review verdict audit failed:")
        for error in errors:
            print(f"- {error}")
        return 1
    print("C2-LT-B9 manual/fulltext fresh-review verdict audit ok: reviewed=8 eligible=4 blocked=4 model=blocked")
    return 0
