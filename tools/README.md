...
- `audit_human_infra_falsifier_source_card_extraction.py`: verifies that the first field-level Source Card extraction slice maps selected source anchors from `docs/reference/human-infra-falsifier-source-card-extraction.json` to exact source identity, Human Infra domains, paper claims, model positions, transfer boundaries and the human-readable source-note pack.
