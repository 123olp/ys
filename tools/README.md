...
- `audit_human_infra_c2_longtail_first_batch_source_extraction_register.py`: verifies that `docs/reference/human-infra-c2-longtail-first-batch-source-extraction-register.json` completes the first 24/48 C2 long-tail source-context extraction rows with required fields, downgrade triggers, blocked uses and index links.
