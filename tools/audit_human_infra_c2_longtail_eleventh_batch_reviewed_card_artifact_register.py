#!/usr/bin/env python3
"""审计 C2 长尾第十一批 bounded reviewed artifact 账本。"""
...
REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-eleventh-batch-reviewed-card-artifact-register.json"
VERDICT_PATH = (
    ROOT
    / "docs/reference/human-infra-c2-longtail-eleventh-batch-independent-fresh-review-verdict-register.json"
)

SCHEMA = "human-infra.c2-longtail-eleventh-batch-reviewed-card-artifact-register.v1"
STATUS = "active-c2ltb11-reviewed-card-artifact-register-model-blocked"
REGISTER_LINK = "human-infra-c2-longtail-eleventh-batch-reviewed-card-artifact-register.json"
AUDIT_SCRIPT = "audit_human_infra_c2_longtail_eleventh_batch_reviewed_card_artifact_register.py"
AUDIT_TARGET = "c2-longtail-eleventh-batch-reviewed-card-artifact-audit"
...
ARTIFACT_DECISION = "filled-from-eligible-c2ltb11-independent-fresh-review"
...
def verdict_rows(errors: list[str]) -> tuple[dict[str, dict[str, Any]], dict[str, dict[str, Any]]]:
    data = load_json(VERDICT_PATH, errors, "C2-LT-B11 independent fresh review verdict register")
    ...
    if len(eligible) != 21:
        fail(errors, "verdict register must contain 21 eligible rows")
    if len(blocked) != 3:
        fail(errors, "verdict register must contain 3 blocked PubMed-only rows")
...
def validate_scope(register: dict[str, Any], errors: list[str]) -> None:
    ...
    if not isinstance(scope, dict):
        ...
        return
    expected_counts = {
        "eligibleReviewRowCount": 21,
        "artifactTypesPerEligibleRow": 6,
        "reviewedArtifactCount": 126,
        "blockedRowCount": 3,
...
def validate_artifacts(
    ...
    if not isinstance(artifacts, list):
        ...
        return
    if len(artifacts) != 126:
        fail(errors, "reviewedArtifacts must contain 126 artifacts")
    ...
    for artifact_type in ARTIFACT_TYPES:
        if per_type[artifact_type] != 21:
            fail(errors, f"{artifact_type} must have exactly 21 artifacts")
...
def validate_blocked_rows(
    ...
    if not isinstance(rows, list):
        ...
        return
    if len(rows) != 3:
        fail(errors, "blockedRows must contain 3 PubMed-only rows")
    seen: set[str] = set()
    for index, row in enumerate(rows):
        if not isinstance(row, dict):
    ...
    for relative_path in REQUIRED_INDEX_FILES:
        ...
        if relative_path == "Makefile":
            if AUDIT_TARGET not in text:
                fail(errors, "Makefile must expose eleventh-batch reviewed artifact audit target")
            if AUDIT_SCRIPT not in text:
                fail(errors, "Makefile must run eleventh-batch reviewed artifact audit script")
...
def main() -> int:
    errors: list[str] = []
    register = load_json(REGISTER_PATH, errors, "C2-LT-B11 reviewed card artifact register")
    ...
    if errors:
        ...
        return 1
    print(
        "C2 longtail eleventh-batch reviewed artifact audit ok: "
        "artifacts=126 eligible_rows=21 blocked_pubmed_only=3 model_admission=0"
    )
    return 0
