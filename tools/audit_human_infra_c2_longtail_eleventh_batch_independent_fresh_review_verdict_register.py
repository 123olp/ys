#!/usr/bin/env python3
"""审计 C2 长尾第十一批 independent fresh-review verdict 账本。"""
...
REGISTER_PATH = (
    ROOT
    / "docs/reference/human-infra-c2-longtail-eleventh-batch-independent-fresh-review-verdict-register.json"
)
PROTOCOL_PATH = ROOT / "docs/reference/human-infra-c2-longtail-eleventh-batch-independent-fresh-review-protocol.json"
EXTRACTION_PATH = ROOT / "docs/reference/human-infra-c2-longtail-eleventh-batch-source-extraction-register.json"

SCHEMA = "human-infra.c2-longtail-eleventh-batch-independent-fresh-review-verdict-register.v1"
STATUS = "active-c2ltb11-independent-fresh-review-verdict-register-model-blocked"
REGISTER_LINK = "human-infra-c2-longtail-eleventh-batch-independent-fresh-review-verdict-register.json"
AUDIT_SCRIPT = "audit_human_infra_c2_longtail_eleventh_batch_independent_fresh_review_verdict_register.py"
AUDIT_TARGET = "c2-longtail-eleventh-batch-independent-fresh-review-verdict-audit"
EXPECTED_TASK_IDS = [f"C2LTB11-EXT-{index:03d}" for index in range(1, 25)]
BLOCKED_TASK_IDS = ["C2LTB11-EXT-022", "C2LTB11-EXT-023", "C2LTB11-EXT-024"]
ELIGIBLE_DECISION = "eligible-for-bounded-reviewed-artifact-prep"
BLOCKED_DECISION = "blocked-cannot-evaluate"
MODEL_DECISION = "blocked-pending-b11-reviewed-artifact-gates-and-calibrated-model-validation"
ARTIFACT_TYPES = [
...
def extraction_rows(errors: list[str]) -> dict[str, dict[str, Any]]:
    data = load_json(EXTRACTION_PATH, errors, "C2-LT-B11 source extraction register")
    ...
    if list(result) != EXPECTED_TASK_IDS:
        fail(errors, "source extraction rows must be ordered as C2LTB11-EXT-001 through C2LTB11-EXT-024")
    return result


def required_blocked_uses(errors: list[str]) -> set[str]:
    protocol = load_json(PROTOCOL_PATH, errors, "C2-LT-B11 independent fresh review protocol")
...
def validate_scope(register: dict[str, Any], errors: list[str]) -> None:
    ...
    expected_counts = {
        ...
        "eligibleForBoundedArtifactPrepCount": 21,
        "downgradeBeforeFillCount": 0,
        "rejectedNoFillCount": 0,
        "blockedCannotEvaluateCount": 3,
        "modelAdmissionCount": 0,
        "adviceUseAdmissionCount": 0,
    }
    if scope.get("batchId") != "C2-LT-B11":
        fail(errors, "scope.batchId must be C2-LT-B11")
    ...
    if require_string_list(scope.get("reviewedTaskIds"), "scope.reviewedTaskIds", errors, 24) != EXPECTED_TASK_IDS:
        fail(errors, "scope.reviewedTaskIds must cover all C2LTB11 task IDs")
    if require_string_list(scope.get("blockedTaskIds"), "scope.blockedTaskIds", errors, 3) != BLOCKED_TASK_IDS:
...
def validate_reviews(register: dict[str, Any], rows_by_task: dict[str, dict[str, Any]], blocked_uses: set[str], errors: list[str]) -> None:
    ...
    for index, review in enumerate(reviews):
        ...
        expected_review_id = f"C2LTB11-FRV-{index + 1:03d}"
        ...
        if review.get("batchId") != "C2-LT-B11":
            fail(errors, f"{review_id}.batchId must be C2-LT-B11")
        ...
        if set(require_string_list(review.get("blockedUses"), f"{review_id}.blockedUses", errors, 20)) != blocked_uses:
    ...
    if seen != EXPECTED_TASK_IDS:
        fail(errors, "freshReviewVerdicts must cover C2LTB11-EXT-001 through C2LTB11-EXT-024 in order")
    if eligible_count != 21 or blocked_count != 3:
        fail(errors, "freshReviewVerdicts must contain 21 eligible rows and 3 blocked PubMed-only rows")
    ...
    for relative_path in REQUIRED_INDEX_FILES:
        ...
        if relative_path == "Makefile":
            if AUDIT_TARGET not in text:
                fail(errors, "Makefile must expose eleventh-batch independent fresh review verdict audit target")
            if AUDIT_SCRIPT not in text:
                fail(errors, "Makefile must run eleventh-batch independent fresh review verdict audit script")
...
def main() -> int:
    errors: list[str] = []
    register = load_json(REGISTER_PATH, errors, "C2-LT-B11 independent fresh review verdict register")
    ...
    if errors:
        ...
        return 1
    print(
        "C2 longtail eleventh-batch independent fresh review verdict audit ok: "
        "reviews=24 eligible=21 blocked_pubmed_only=3 model_admission=0"
    )
    return 0
