...
REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-seventh-batch-source-extraction-register.json"
QUEUE_PATH = ROOT / "docs/reference/human-infra-c2-longtail-seventh-batch-source-extraction-queue.json"

SCHEMA = "human-infra.c2-longtail-seventh-batch-source-extraction-register.v1"
STATUS = "active-c2ltb7-source-extractions-pending-fresh-review"
REGISTER_LINK = "human-infra-c2-longtail-seventh-batch-source-extraction-register.json"
...
def validate_source_of_truth(data: dict[str, Any], errors: list[str]) -> None:
    ...
    if not isinstance(source, dict):
        ...
        return
    for key in ["sourceExtractionQueue", "sourceCardSystem", "maturityGapRegister", "maturityRoadmap"]:
...
def validate_scope(scope: Any, rows: list[dict[str, Any]], queue_count: int, errors: list[str]) -> None:
    ...
    if selected != EXPECTED_TASK_IDS:
        fail(errors, "scope.selectedTaskIds must equal all 24 C2LTB7 extraction task IDs")
...
def queue_tasks(errors: list[str]) -> dict[str, dict[str, Any]]:
    queue = load_json(QUEUE_PATH, errors, "C2 seventh-batch source extraction queue")
...
def validate_rows(rows: Any, task_map: dict[str, dict[str, Any]], errors: list[str]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(rows):
        ...
        if task is None:
            ...
            continue
        if row.get("domainId") != task.get("domainId"):
            fail(errors, f"{task_id}.domainId must match queue")
        if row.get("localDomainPath") != task.get("localDomainPath"):
            fail(errors, f"{task_id}.localDomainPath must match queue")
        if row.get("sourceRefId") != task.get("sourceRefId"):
            fail(errors, f"{task_id}.sourceRefId must match queue")
        if row.get("sourceTitle") != task.get("sourceTitle"):
            fail(errors, f"{task_id}.sourceTitle must match queue")
        if row.get("sourceUrl") != task.get("sourceUrl"):
            fail(errors, f"{task_id}.sourceUrl must match queue")
    ...
    if [row.get("taskId") for row in result] != EXPECTED_TASK_IDS:
        fail(errors, "extractedRows must be ordered as C2LTB7-EXT-001 through C2LTB7-EXT-024")
    return result


def validate_index_links(data: dict[str, Any], errors: list[str]) -> None:
    requirements = require_string_list(data.get("indexRequirements"), "indexRequirements", errors, 4)
    for relative_path in ["README.md", "docs/reference/README.md", "docs/reference/human-infra-maturity-roadmap.md", "tools/README.md"]:
    ...
    if len(requirements) < 4:
        fail(errors, "indexRequirements must list at least four index targets")


def main() -> int:
    errors: list[str] = []
    data = load_json(REGISTER_PATH, errors, "C2 seventh-batch source extraction register")
    ...
    require_string_list(data.get("sourceAccessNotes"), "sourceAccessNotes", errors, 2)
    ...
    print(
        "C2 longtail seventh-batch source extraction register audit ok: "
