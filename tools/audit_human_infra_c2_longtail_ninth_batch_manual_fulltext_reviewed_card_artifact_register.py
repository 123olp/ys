#!/usr/bin/env python3
"""审计 C2-LT-B9 manual/fulltext reviewed artifact 账本。"""
...
REGISTER_PATH = (
    ROOT
    / "docs/reference/human-infra-c2-longtail-ninth-batch-manual-fulltext-reviewed-card-artifact-register.json"
)
VERDICT_PATH = (
    ROOT
...
SCHEMA = "human-infra.c2ltb9-manual-fulltext-reviewed-card-artifact-register.v1"
STATUS = "active-c2ltb9-manual-fulltext-reviewed-card-artifact-register-model-blocked"
REGISTER_LINK = "human-infra-c2-longtail-ninth-batch-manual-fulltext-reviewed-card-artifact-register.json"
SCRIPT_LINK = "audit_human_infra_c2_longtail_ninth_batch_manual_fulltext_reviewed_card_artifact_register.py"
BATCH_ID = "C2-LT-B9"
...
MODEL_DECISION = "blocked-pending-c2ltb9-reviewed-artifact-gates-and-calibrated-model-validation"
...
def validate_scope(
    ...
    if scope.get("artifactLevel") != "c2ltb9-manual-fulltext-fresh-review-verdict-to-reviewed-card-artifact":
...
def validate_artifacts(
    ...
    for index, artifact in enumerate(artifacts):
        ...
        if artifact.get("artifactStatus") != "filled-reviewed-c2ltb9-manual-fulltext-artifact-model-blocked":
...
def main() -> int:
    errors: list[str] = []
    register = load_json(REGISTER_PATH, errors, "B9 manual/fulltext reviewed artifact register")
    ...
    if errors:
        print("C2-LT-B9 manual/fulltext reviewed artifact audit failed:")
        for error in errors:
            print(f"- {error}")
        return 1
    print("C2-LT-B9 manual/fulltext reviewed artifact audit ok: artifacts=24 blocked=4 model=blocked")
    return 0
