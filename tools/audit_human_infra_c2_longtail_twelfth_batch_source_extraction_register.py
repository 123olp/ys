...
REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-twelfth-batch-source-extraction-register.json"
QUEUE_PATH = ROOT / "docs/reference/human-infra-c2-longtail-twelfth-batch-source-extraction-queue.json"

SCHEMA = "human-infra.c2-longtail-twelfth-batch-source-extraction-register.v1"
STATUS = "active-c2ltb12-source-extractions-pending-local-review-and-fresh-review"
REGISTER_LINK = "human-infra-c2-longtail-twelfth-batch-source-extraction-register.json"
AUDIT_SCRIPT = "audit_human_infra_c2_longtail_twelfth_batch_source_extraction_register.py"
AUDIT_TARGET = "c2-longtail-twelfth-batch-source-extraction-register-audit"
...
def validate_scope(scope: Any, rows: list[dict[str, Any]], queue_count: int, errors: list[str]) -> None:
    ...
    if selected != EXPECTED_TASK_IDS:
        fail(errors, "scope.selectedTaskIds must equal all 24 C2LTB12 extraction task IDs")
...
def queue_tasks(errors: list[str]) -> dict[str, dict[str, Any]]:
    queue = load_json(QUEUE_PATH, errors, "C2 twelfth-batch source extraction queue")
...
def validate_rows(rows: Any, task_map: dict[str, dict[str, Any]], errors: list[str]) -> list[dict[str, Any]]:
    ...
    if [row.get("taskId") for row in result] != EXPECTED_TASK_IDS:
        fail(errors, "extractedRows must be ordered as C2LTB12-EXT-001 through C2LTB12-EXT-024")
    return result
...
def validate_index_links(data: dict[str, Any], errors: list[str]) -> None:
    ...
    if not makefile.exists():
        ...
        if AUDIT_TARGET not in make_text:
            fail(errors, "Makefile does not expose twelfth-batch source extraction register audit target")
        if AUDIT_SCRIPT not in make_text:
            fail(errors, "Makefile does not run twelfth-batch source extraction register audit script")
...
def main() -> int:
    errors: list[str] = []
    data = load_json(REGISTER_PATH, errors, "C2 twelfth-batch source extraction register")
    ...
    print(
        "C2 longtail twelfth-batch source extraction register audit ok: "
