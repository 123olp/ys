...
REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-fourteenth-batch-local-review-register.json"
QUEUE_PATH = ROOT / "docs/reference/human-infra-c2-longtail-fourteenth-batch-source-extraction-queue.json"
EXTRACTION_REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-fourteenth-batch-source-extraction-register.json"

SCHEMA = "human-infra.c2-longtail-fourteenth-batch-local-review-register.v1"
STATUS = "complete-local-source-context-review-source-resolution-and-independent-fresh-review-required"
REGISTER_LINK = "human-infra-c2-longtail-fourteenth-batch-local-review-register.json"
AUDIT_SCRIPT = "audit_human_infra_c2_longtail_fourteenth_batch_local_review_register.py"
AUDIT_TARGET = "c2-longtail-fourteenth-batch-local-review-audit"
...
REQUIRED_NEXT_ARTIFACTS = {
    "c2ltb14-source-resolution-register",
    "c2ltb14-independent-fresh-review-protocol",
    "c2ltb14-independent-fresh-review-verdict-register",
...
def load_queue(errors: list[str]) -> dict[str, dict[str, Any]]:
    queue = load_json(QUEUE_PATH, errors, "C2-LT-B14 source extraction queue")
    ...
    if list(result) != EXPECTED_TASK_IDS:
        fail(errors, "queue task IDs must be C2LTB14-EXT-001 through C2LTB14-EXT-032")
    return result


def load_extraction_rows(errors: list[str]) -> dict[str, dict[str, Any]]:
    register = load_json(EXTRACTION_REGISTER_PATH, errors, "C2-LT-B14 source extraction register")
    ...
    for index, row in enumerate(rows):
        ...
        if task_id:
            result[task_id] = row
    if list(result) != EXPECTED_TASK_IDS:
        fail(errors, "source extraction register rows must be ordered as all 32 C2LTB14 task IDs")
    return result
...
def validate_source_of_truth(data: dict[str, Any], errors: list[str]) -> None:
    ...
    if not isinstance(source, dict):
        ...
        return
    for key in ["sourceExtractionQueue", "sourceExtractionRegister", "sourceCardSystem", "maturityGapRegister", "maturityRoadmap"]:
...
def validate_scope(scope: Any, task_map: dict[str, dict[str, Any]], errors: list[str]) -> None:
    ...
    expected_counts = {
        ...
        "independentFreshReviewCandidateTaskCount": len(EXPECTED_TASK_IDS) - len(EXPECTED_SOURCE_RESOLUTION_IDS),
    }
    ...
    if selected != EXPECTED_TASK_IDS:
        fail(errors, "scope.reviewedExtractionTaskIds must equal all 32 C2LTB14 task IDs")
    issue_ids = require_string_list(
        scope.get("sourceResolutionIssueTaskIds"),
        "scope.sourceResolutionIssueTaskIds",
        errors,
        len(EXPECTED_SOURCE_RESOLUTION_IDS),
    )
    if issue_ids != EXPECTED_SOURCE_RESOLUTION_IDS:
        fail(errors, "scope.sourceResolutionIssueTaskIds must preserve all B14 manual/source-resolution rows")
...
def validate_batches(
    ...
    for index, batch in enumerate(batches):
        ...
        if require_string(batch.get("reviewDecision"), f"{context}.reviewDecision", errors) != "local-review-mixed-pass-and-source-resolution-required":
            fail(errors, f"{context}.reviewDecision must preserve mixed pass/source-resolution state")
    ...
    if covered != EXPECTED_TASK_IDS:
        fail(errors, "reviewBatches must cover all 32 task IDs exactly once in order")
    ...
    for index, finding in enumerate(findings):
        ...
        if task_id:
            ...
            if row is None:
                ...
                markers = ["403", "404", "manual", "source-resolution", "access", "route"]
                if not any(marker in source_context for marker in markers):
                    fail(errors, f"{task_id} must preserve manual-route/source-resolution context")
        ...
        if not any(marker in disposition for marker in ["source-resolution", "manual-route", "bounded-fresh-review"]):
            fail(errors, f"{context}.reviewDisposition must preserve source-resolution/manual-route boundary")
    if task_ids != EXPECTED_SOURCE_RESOLUTION_IDS:
        fail(errors, f"sourceResolutionFindings must equal {EXPECTED_SOURCE_RESOLUTION_IDS}")
...
def validate_register_fields(data: dict[str, Any], errors: list[str]) -> None:
    ...
    fields = require_string_list(data.get("requiredExtractionFieldsReviewed"), "requiredExtractionFieldsReviewed", errors, len(REQUIRED_ROW_FIELDS))
    if set(fields) != set(REQUIRED_ROW_FIELDS):
        fail(errors, "requiredExtractionFieldsReviewed must match all required B14 extraction row fields")
    ...
    if "source-resolution-required" not in str(data.get("sourceResolutionDecision", "")):
        fail(errors, "sourceResolutionDecision must require source resolution for B14 issue rows")
    ...
    require_string_list(data.get("localReviewChecks"), "localReviewChecks", errors, 8)
...
def validate_index_links(errors: list[str]) -> None:
    ...
    if not makefile.exists():
        ...
        if AUDIT_TARGET not in make_text:
            fail(errors, "Makefile does not expose fourteenth-batch local review audit target")
        if AUDIT_SCRIPT not in make_text:
            fail(errors, "Makefile does not run fourteenth-batch local review audit script")


def main() -> int:
    errors: list[str] = []
    data = load_json(REGISTER_PATH, errors, "C2-LT-B14 local review register")
    ...
    if errors:
        ...
        return 1
    print(
        "C2 longtail fourteenth-batch local review register audit ok: "
        "batch=C2-LT-B14 reviewed=32 domains=16 source_resolution_issues=8"
    )
    return 0
