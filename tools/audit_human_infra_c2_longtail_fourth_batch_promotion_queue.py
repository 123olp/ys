...
QUEUE_PATH = ROOT / "docs/reference/human-infra-c2-longtail-fourth-batch-promotion-queue.json"
...
SCHEMA = "human-infra.c2-longtail-fourth-batch-promotion-queue.v1"
STATUS = "active-fourth-batch-queue-model-blocked"
EXPECTED_BATCH_ID = "C2-LT-B4"
EXPECTED_COVERAGE_STATUS = "c2-longtail-uncovered"
REGISTER_LINK = "human-infra-c2-longtail-fourth-batch-promotion-queue.json"
SCRIPT_LINK = "audit_human_infra_c2_longtail_fourth_batch_promotion_queue.py"
...
def validate_candidate_source(ref: Any, path: str, seen_refs: set[str], errors: list[str]) -> None:
    ...
    if source_ref_id:
        ...
        if not source_ref_id.startswith("C2LTB4-SRC-"):
            fail(errors, f"{path}.sourceRefId must start with C2LTB4-SRC-")
...
def validate_row(
    ...
    if domain_id in previous_batch_domains:
        fail(errors, f"{domain_id} must not duplicate C2-LT-B1, C2-LT-B2 or C2-LT-B3")
...
def validate_summary(data: dict[str, Any], rows: list[Any], errors: list[str]) -> None:
    ...
    if summary.get("batchId") != EXPECTED_BATCH_ID:
        fail(errors, "summary.batchId must be C2-LT-B4")
...
def main() -> int:
    errors: list[str] = []
    queue = load_json(QUEUE_PATH, errors, "C2 longtail fourth-batch queue")
    ...
    previous_batch_domains = first_batch_domains | second_batch_domains | third_batch_domains
    ...
    if errors:
        print("C2 longtail fourth-batch promotion queue audit failed:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    print(
        "C2 longtail fourth-batch promotion queue audit ok: "
        f"batch=C2-LT-B4 domains={len(seen_domains)} candidate_sources={len(seen_refs)}"
    )
    return 0
