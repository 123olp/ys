#!/usr/bin/env python3
"""审计 C2-LT-B9 manual/fulltext extraction 账本。"""
...
REGISTER_PATH = (
    ROOT / "docs/reference/human-infra-c2-longtail-ninth-batch-manual-fulltext-extraction-register.json"
)
SOURCE_RESOLUTION_PATH = (
    ROOT / "docs/reference/human-infra-c2-longtail-ninth-batch-source-resolution-register.json"
)

SCHEMA = "human-infra.c2ltb9-manual-fulltext-extraction-register.v1"
STATUS = "active-c2ltb9-manual-fulltext-extracted-fresh-review-required-model-blocked"
REGISTER_LINK = "human-infra-c2-longtail-ninth-batch-manual-fulltext-extraction-register.json"
SCRIPT_LINK = "audit_human_infra_c2_longtail_ninth_batch_manual_fulltext_extraction_register.py"
BATCH_ID = "C2-LT-B9"
...
def validate_scope(register: dict[str, Any], rows: list[dict[str, Any]], errors: list[str]) -> None:
    ...
    if not isinstance(scope, dict):
        ...
        return
    expected = {
        "sourceResolutionIssueRowCount": 2,
        ...
        "routeOnlyOrProvenanceBlockedRowCount": len(EXPECTED_BLOCKED_ROWS),
    ...
    if scope.get("manualExtractionLevel") != "c2ltb9-source-resolution-manual-fulltext-extraction-v0.1":
    ...
    for phrase in [
        "not an independent fresh review",
        "does not create reviewed artifacts",
        "does not authorize",
        "does not open calibrated prediction",
    ]:
...
def validate_summary(register: dict[str, Any], errors: list[str]) -> None:
    ...
    ready = set(
        require_string_list(
            summary.get("boundedFreshReviewCandidateRows"),
            "summary.boundedFreshReviewCandidateRows",
            errors,
            len(EXPECTED_READY_ROWS),
        )
    )
    blocked = set(
        require_string_list(
            summary.get("routeOnlyOrProvenanceBlockedRows"),
            "summary.routeOnlyOrProvenanceBlockedRows",
            errors,
            len(EXPECTED_BLOCKED_ROWS),
        )
    )
    ...
    if blocked != EXPECTED_BLOCKED_ROWS:
        fail(errors, "summary.routeOnlyOrProvenanceBlockedRows must match expected blocked rows")
...
def validate_rows(
    ...
    for index, row in enumerate(rows):
        ...
        if row_id and row_id != f"C2LTB9-MFEXT-{index + 1:03d}":
            fail(errors, f"{row_id} must follow sequential C2LTB9-MFEXT-### order")
        ...
        if row.get("extractionDate") != EXTRACTION_DATE:
            fail(errors, f"{row_id}.extractionDate must be {EXTRACTION_DATE}")
        ...
        if not any(term in str(row.get("exactClaimUse", "")) for term in ["Use only", "only"]):
            fail(errors, f"{row_id}.exactClaimUse must be bounded")
    ...
    if seen_ids != EXPECTED_READY_ROWS | EXPECTED_BLOCKED_ROWS:
        fail(errors, "manualExtractionRows IDs must match expected C2-LT-B9 manual extraction rows")
    return valid_rows
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    print(
        "C2-LT-B9 manual/fulltext extraction register audit ok: "
