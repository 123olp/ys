#!/usr/bin/env python3
"""审计 C2-LT-B6 source-resolution 账本。"""
...
REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-sixth-batch-source-resolution-register.json"
QUEUE_PATH = ROOT / "docs/reference/human-infra-c2-longtail-sixth-batch-source-extraction-queue.json"
EXTRACTION_PATH = ROOT / "docs/reference/human-infra-c2-longtail-sixth-batch-source-extraction-register.json"
LOCAL_REVIEW_PATH = ROOT / "docs/reference/human-infra-c2-longtail-sixth-batch-local-review-register.json"

SCHEMA = "human-infra.c2-longtail-sixth-batch-source-resolution-register.v1"
STATUS = "active-c2ltb6-source-resolution-register-independent-fresh-review-required"
REGISTER_LINK = "human-infra-c2-longtail-sixth-batch-source-resolution-register.json"
SCRIPT_LINK = "audit_human_infra_c2_longtail_sixth_batch_source_resolution_register.py"
...
GUIDELINE_ROUTE_TASK_IDS = {
    "C2LTB6-EXT-007",
    "C2LTB6-EXT-010",
    "C2LTB6-EXT-012",
}
PUBLISHER_MANUAL_TASK_IDS = {
    "C2LTB6-EXT-008",
    "C2LTB6-EXT-013",
    "C2LTB6-EXT-015",
    "C2LTB6-EXT-020",
}
SOURCE_LINEAGE_TASK_IDS = {
    "C2LTB6-EXT-010",
    "C2LTB6-EXT-012",
}
EXPECTED_CANDIDATE_COUNT = 19
EXPECTED_VERIFICATION_IDS = {
    "C2LTB6-SR-VERIFY-ESHRE-POI-20260703",
    "C2LTB6-SR-VERIFY-ACOG-POI-20260703",
    "C2LTB6-SR-VERIFY-PCOS-GUIDELINE-LINEAGE-20260703",
    "C2LTB6-SR-VERIFY-ENDOMETRIOSIS-GUIDELINE-LINEAGE-20260703",
    "C2LTB6-SR-VERIFY-ACOG-EPL-20260703",
    "C2LTB6-SR-VERIFY-AAP-BREASTFEEDING-20260703",
    "C2LTB6-SR-VERIFY-AAP-LEAD-20260703",
}
...
def validate_scope(data: dict[str, Any], rows: list[dict[str, Any]], errors: list[str]) -> None:
    ...
    expected = {
        ...
        "guidelineRouteIssueCount": len(GUIDELINE_ROUTE_TASK_IDS),
        "publisherManualReviewIssueCount": len(PUBLISHER_MANUAL_TASK_IDS),
        "sourceLineageIssueCount": len(SOURCE_LINEAGE_TASK_IDS),
        "resolutionCandidateCount": EXPECTED_CANDIDATE_COUNT,
    }
    if scope.get("batchId") != "C2-LT-B6":
        fail(errors, "scope.batchId must be C2-LT-B6")
    if scope.get("resolutionLevel") != "c2ltb6-local-review-issue-row-source-resolution-v0.1":
...
def validate_candidates(row: dict[str, Any], index: int, verification_ids: set[str], errors: list[str]) -> None:
    ...
    for cand_index, candidate in enumerate(candidates):
        ...
        if isinstance(boundary, str) and not any(
            phrase in boundary for phrase in ["no artifact fill", "no individual", "Use only"]
        ):
...
def validate_resolution_rows(data: dict[str, Any], verification_ids: set[str], errors: list[str]) -> None:
    ...
    if [row.get("taskId") for row in rows if isinstance(row, dict)] != EXPECTED_TASK_IDS:
        fail(errors, "resolutionRows task order must match expected C2-LT-B6 issue rows")
    ...
    for index, row in enumerate(rows):
        ...
        if task_id in extraction_rows:
            expected_domain = extraction_rows[task_id].get("domainId")
            if row.get("domainId") != expected_domain:
                fail(errors, f"{task_id}.domainId must match extraction register")
...
def main() -> int:
    ...
    print(
        "C2-LT-B6 source-resolution register audit ok: "
