#!/usr/bin/env python3
"""审计 C2-LT-B6 manual/fulltext extraction 账本。"""
...
SOURCE_RESOLUTION_PATH = (
    ROOT / "docs/reference/human-infra-c2-longtail-sixth-batch-source-resolution-register.json"
)

SCHEMA = "human-infra.c2ltb6-manual-fulltext-extraction-register.v1"
STATUS = "active-c2ltb6-manual-fulltext-extracted-fresh-review-required-model-blocked"
REGISTER_LINK = "human-infra-c2-longtail-sixth-batch-manual-fulltext-extraction-register.json"
SCRIPT_LINK = "audit_human_infra_c2_longtail_sixth_batch_manual_fulltext_extraction_register.py"
BATCH_ID = "C2-LT-B6"
...
def validate_scope(register: dict[str, Any], rows: list[dict[str, Any]], errors: list[str]) -> None:
    ...
    if scope.get("manualExtractionLevel") != "c2ltb6-source-resolution-manual-fulltext-extraction-v0.1":
...
def validate_rows(
    ...
    for index, row in enumerate(rows):
        ...
        if row_id and row_id != f"C2LTB6-MFEXT-{index + 1:03d}":
            fail(errors, f"{row_id} must follow sequential C2LTB6-MFEXT-### order")
    ...
    if seen_ids != EXPECTED_READY_ROWS | EXPECTED_BLOCKED_ROWS:
        fail(errors, "manualExtractionRows IDs must match expected C2-LT-B6 manual extraction rows")
    return valid_rows
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    print(
        "C2-LT-B6 manual/fulltext extraction register audit ok: "
