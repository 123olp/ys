...
REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-first-batch-source-extraction-register.json"
...
EXPECTED_TASK_IDS = [f"C2LTB1-EXT-{index:03d}" for index in range(1, 49)]
...
def validate_scope(scope: Any, rows: list[dict[str, Any]], queue_count: int, errors: list[str]) -> None:
    ...
    if selected != EXPECTED_TASK_IDS:
        fail(errors, "scope.selectedTaskIds must equal all 48 C2LTB1 extraction task IDs")
...
def queue_tasks(errors: list[str]) -> dict[str, dict[str, Any]]:
    queue = load_json(QUEUE_PATH, errors, "C2 first-batch source extraction queue")
...
def validate_rows(rows: Any, task_map: dict[str, dict[str, Any]], errors: list[str]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(rows):
        ...
        if row.get("extractionStatus") != "completed-c2ltb1-pilot-source-context-extraction":
            fail(errors, f"{task_id}.extractionStatus must be completed-c2ltb1-pilot-source-context-extraction")
        for field in [
            "sourceAccessStatus",
            "sourceRole",
            "exactClaimUse",
    ...
    if [row.get("taskId") for row in result] != EXPECTED_TASK_IDS:
        fail(errors, "extractedRows must be ordered as C2LTB1-EXT-001 through C2LTB1-EXT-012")
    return result
...
def main() -> int:
    errors: list[str] = []
    data = load_json(REGISTER_PATH, errors, "C2 first-batch source extraction register")
    ...
    print(
        "C2 longtail first-batch source extraction register audit ok: "
