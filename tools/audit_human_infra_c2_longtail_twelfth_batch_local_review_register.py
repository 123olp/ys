...
REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-twelfth-batch-local-review-register.json"
QUEUE_PATH = ROOT / "docs/reference/human-infra-c2-longtail-twelfth-batch-source-extraction-queue.json"
EXTRACTION_REGISTER_PATH = ROOT / "docs/reference/human-infra-c2-longtail-twelfth-batch-source-extraction-register.json"

SCHEMA = "human-infra.c2-longtail-twelfth-batch-local-review-register.v1"
STATUS = "complete-local-source-context-review-independent-fresh-review-required"
REGISTER_LINK = "human-infra-c2-longtail-twelfth-batch-local-review-register.json"
AUDIT_SCRIPT = "audit_human_infra_c2_longtail_twelfth_batch_local_review_register.py"
AUDIT_TARGET = "c2-longtail-twelfth-batch-local-review-audit"
...
REQUIRED_NEXT_ARTIFACTS = {
    "c2ltb12-independent-fresh-review-protocol",
    "c2ltb12-independent-fresh-review-verdict-register",
...
def load_queue(errors: list[str]) -> dict[str, dict[str, Any]]:
    queue = load_json(QUEUE_PATH, errors, "C2-LT-B12 source extraction queue")
    ...
    if list(result) != EXPECTED_TASK_IDS:
        fail(errors, "queue task IDs must be C2LTB12-EXT-001 through C2LTB12-EXT-024")
    return result


def load_extraction_rows(errors: list[str]) -> dict[str, dict[str, Any]]:
    register = load_json(EXTRACTION_REGISTER_PATH, errors, "C2-LT-B12 source extraction register")
    ...
    for index, row in enumerate(rows):
        ...
        if task_id:
            result[task_id] = row
    if list(result) != EXPECTED_TASK_IDS:
        fail(errors, "source extraction register rows must be ordered as all 24 C2LTB12 task IDs")
    return result
...
def validate_scope(scope: Any, task_map: dict[str, dict[str, Any]], errors: list[str]) -> None:
    if not isinstance(scope, dict):
        ...
        return
    if require_string(scope.get("batchId"), "scope.batchId", errors) != "C2-LT-B12":
    ...
    if selected != EXPECTED_TASK_IDS:
        fail(errors, "scope.reviewedExtractionTaskIds must equal all 24 C2LTB12 task IDs")
...
def validate_extraction_row(row: dict[str, Any], task: dict[str, Any], errors: list[str]) -> None:
    ...
    if row.get("extractionStatus") != "completed-c2ltb12-source-context-extraction":
        fail(errors, f"{task_id}.extractionStatus must be completed-c2ltb12-source-context-extraction")
    for field in [
        "sourceRole",
        "exactClaimUse",
        "endpointDefinition",
...
def validate_source_resolution_findings(findings: Any, extraction_rows: dict[str, dict[str, Any]], errors: list[str]) -> None:
    ...
    if task_ids != EXPECTED_SOURCE_RESOLUTION_IDS:
        fail(errors, "sourceResolutionFindings must be empty for C2-LT-B12 local review")
...
def validate_index_links(errors: list[str]) -> None:
    ...
    if not makefile.exists():
        ...
        if AUDIT_TARGET not in make_text:
            fail(errors, "Makefile does not expose twelfth-batch local review audit target")
        if AUDIT_SCRIPT not in make_text:
            fail(errors, "Makefile does not run twelfth-batch local review audit script")


def main() -> int:
    errors: list[str] = []
    data = load_json(REGISTER_PATH, errors, "C2-LT-B12 local review register")
    ...
    if errors:
        ...
        return 1
    print(
        "C2 longtail twelfth-batch local review register audit ok: "
        "batch=C2-LT-B12 reviewed=24 domains=12 source_resolution_issues=0"
    )
    return 0
