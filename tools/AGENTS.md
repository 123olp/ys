...
- `audit_human_infra_independent_fresh_review_verdict_register.py` 只检查独立 fresh review verdict 账本是否把 FRB-01 的 5 个来源锚点和 26 个晋升包绑定到 protocol、prep register、本地来源复核、source-specific extraction、阻塞用途和入口索引；它不证明卡片已经填充，也不打开模型准入。
