...
- `audit_human_infra_source_context_local_review_register.py` 只检查当前 6 个本地复核方法、衰老机制和健康价值基础锚点是否反查到 promotion queue、来源证据、受影响任务、阻塞用途和入口索引；它不证明独立 fresh review 已完成，也不允许个体预测、干预排序或临床有效性声明。
