...
QUEUE_PATH = ROOT / "docs/reference/human-infra-c2-longtail-seventh-batch-promotion-queue.json"
...
SCHEMA = "human-infra.c2-longtail-seventh-batch-promotion-queue.v1"
STATUS = "active-seventh-batch-queue-model-blocked"
BATCH_ID = "C2-LT-B7"
EXPECTED_COVERAGE_STATUS = "c2-longtail-uncovered"
REGISTER_LINK = "human-infra-c2-longtail-seventh-batch-promotion-queue.json"
SCRIPT_LINK = "audit_human_infra_c2_longtail_seventh_batch_promotion_queue.py"
...
REQUIRED_INDEX_LINKS = {
    ...
    "Makefile": "c2-longtail-seventh-batch-promotion-audit",
...
def validate_scope(data: dict[str, Any], row_count: int, errors: list[str]) -> None:
    ...
    for phrase in [
        ...
        "does not mark any C2-LT-B1/B2/B3/B4/B5/B6",
    ]:
...
def validate_candidate_source(ref: Any, context: str, seen_refs: set[str], errors: list[str]) -> None:
    ...
    if source_id:
        ...
        if not source_id.startswith("C2LTB7-SRC-"):
            fail(errors, f"{context}.sourceRefId must start with C2LTB7-SRC-")
...
def validate_row(
    ...
    if domain_id in previous:
        fail(errors, f"{domain_id} must not duplicate C2-LT-B1/B2/B3/B4/B5/B6")
    ...
    if row.get("modelAdmissionDecision") != "blocked-pending-c2-longtail-seventh-batch-source-card-promotion":
...
def validate() -> list[str]:
    errors: list[str] = []
    data = load_json(QUEUE_PATH, errors, "seventh batch promotion queue")
    if not data:
        return errors
    ...
    if not isinstance(aggregate, dict):
        ...
        expected = {
            ...
            "nextRequiredGate": "c2-longtail-seventh-batch-source-extraction-queue",
        }
...
def main() -> int:
    errors = validate()
    if errors:
        print("model C2 longtail seventh batch promotion audit failed:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    print("c2 longtail seventh batch promotion audit ok: domains=12 sources=24 model_admission=blocked")
    return 0
