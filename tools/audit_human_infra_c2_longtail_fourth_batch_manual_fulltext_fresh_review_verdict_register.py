#!/usr/bin/env python3
"""审计 C2-LT-B4 manual/fulltext fresh-review 判定账本。"""
...
SCHEMA = "human-infra.c2ltb4-manual-fulltext-fresh-review-verdict-register.v1"
STATUS = "active-c2ltb4-manual-fulltext-fresh-review-verdict-register-model-blocked"
REGISTER_LINK = "human-infra-c2-longtail-fourth-batch-manual-fulltext-fresh-review-verdict-register.json"
SCRIPT_LINK = "audit_human_infra_c2_longtail_fourth_batch_manual_fulltext_fresh_review_verdict_register.py"
BATCH_ID = "C2-LT-B4"
...
BLOCKED_PROMOTION_DECISIONS = {
    ...
    "blocked-route-context-no-direct-artifact-fill",
}

EXPECTED_ELIGIBLE_ROWS = {"C2LTB4-MFEXT-002", "C2LTB4-MFEXT-005", "C2LTB4-MFEXT-007"}
...
def validate_reviews(
    ...
    for index, review in enumerate(reviews):
        ...
        if review_id and review_id != f"C2LTB4-MFFRV-{index + 1:03d}":
            fail(errors, f"{review_id} must follow sequential C2LTB4-MFFRV-### order")
...
def validate_scope(register: dict[str, Any], reviews: list[dict[str, Any]], errors: list[str]) -> None:
    ...
    if scope.get("reviewLevel") != "c2ltb4-manual-fulltext-fresh-review-v0.1":
...
def validate_summary(register: dict[str, Any], reviews: list[dict[str, Any]], errors: list[str]) -> None:
    ...
    for phrase in ("duplicate-lineage", "ESC", "AASLD", "AGA"):
...
def main() -> int:
    ...
    if errors:
        print("C2-LT-B4 manual/fulltext fresh-review verdict audit failed:")
        for error in errors:
            print(f"- {error}")
        return 1
    print("C2-LT-B4 manual/fulltext fresh-review verdict audit ok: reviewed=8 eligible=3 blocked=5 model=blocked")
    return 0
